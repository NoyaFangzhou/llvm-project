...
#define DEBUG_TYPE "model-codegen"
...
char ModelCodeGenWrapperPass::ID = 0;
static RegisterPass<ModelCodeGenWrapperPass>
    X("model-codegen", "Pass that generate the model code");

ModelCodeGenWrapperPass::ModelCodeGenWrapperPass() : FunctionPass(ID) {}

// the trip is represented as the (ub - lb + 1) / step if <=/>= or
// (ub - lb) / step if </>
// default, the trip is (ub - lb), assuming < and step is 1
string ModelCodeGenWrapperPass::RepresentLoopTrip(LoopTNode *LN)
{
  string trip = "";
...
int ModelCodeGenWrapperPass::ComputeDistanceToParallelLoop(LoopTNode *LN)
{
...
int ModelCodeGenWrapperPass::isInsideParallelLoopNest(PHINode *Phi)
{
...
ReuseType ModelCodeGenWrapperPass::isInterThreadSharingRef(RefTNode *RefNode)
{
  // [i][i], [i][j], [j][k]
...
string ModelCodeGenWrapperPass::ReplaceInductionVarExprByType(
...
string ModelCodeGenWrapperPass::EmitRefNodeAccessExprAtIdx(Value *Index,
                                                             bool isInParallel) {
  TranslateStatus status = SUCCESS;
...
vector<string> ModelCodeGenWrapperPass::EmitRefNodeAccessExpr(RefTNode *RefNode,
                                                                bool isInParallel)
{
...
string ModelCodeGenWrapperPass::EmitBranchCondExpr(BranchTNode *Branch)
{
  TranslateStatus status = SUCCESS;
  string cond = "true";
  stringstream  ss;
...
string ModelCodeGenWrapperPass::EmitLoopNodeExpr(LoopTNode *LoopNode,
                                                   bool isSampledLoop)
{
  TranslateStatus status = SUCCESS;
...
string ModelCodeGenWrapperPass::EmitOneParallelIterationTripFormula(LoopTNode *LoopNode)
{
  if (!LoopNode)
    return "0";
  stringstream ss;
...
string ModelCodeGenWrapperPass::EmitTripFormula(SPSTNode *Node)
{
  string trip_formula = "";
...
string ModelCodeGenWrapperPass::EmitLoopTripExpr(LoopTNode *L)
{
...
bool ModelCodeGenWrapperPass::canBeSimplifiedByFormula(SPSTNode *Node)
{
...
void ModelCodeGenWrapperPass::ModelUtilGen(bool isUniform)
{
  if (isUniform) {
    CodeGen->EmitCode("int getThreadID(int i, int start, int step)");
    CodeGen->EmitCode("{");
    CodeGen->tab_count++;
    CodeGen->EmitStmt(
    ...
    CodeGen->tab_count--;
    CodeGen->EmitCode("}");
    CodeGen->EmitCode("int getChunkID(int i, int start, int step)");
    CodeGen->EmitCode("{");
    CodeGen->tab_count++;
    CodeGen->EmitStmt(
    ...
    CodeGen->tab_count--;
    CodeGen->EmitCode("}");
    CodeGen->EmitCode("int getThreadLocalPos(int i, int start, int step)");
    CodeGen->EmitCode("{");
    CodeGen->tab_count++;
    CodeGen->EmitStmt("return ((i-start)/step) % CHUNK_SIZE");
    CodeGen->tab_count--;
    CodeGen->EmitCode("}");
    CodeGen->EmitCode("int getChunkNum(int trip)");
    CodeGen->EmitCode("{");
    CodeGen->tab_count++;
    CodeGen->EmitStmt(
        "return (trip % (CHUNK_SIZE*THREAD_NUM)) == 0 ? trip / (CHUNK_SIZE*THREAD_NUM) : trip / (CHUNK_SIZE*THREAD_NUM) + 1");
    CodeGen->tab_count--;
    CodeGen->EmitCode("}");
    CodeGen->EmitCode("int hasCompleteChunk(int trip)");
    CodeGen->EmitCode("{");
    CodeGen->tab_count++;
    CodeGen->EmitStmt("return trip % (CHUNK_SIZE * THREAD_NUM)");
    CodeGen->tab_count--;
    CodeGen->EmitCode("}");
  } else {
...
//    CodeGen->EmitStmt("cout << prob_sum << endl");
    ...
    CodeGen->EmitCode("} else {");
    CodeGen->tab_count++;
    CodeGen->EmitFunctionCall("pluss_histogram_update", {"reuse_entry.first", "reuse_entry.second"});
    CodeGen->tab_count--;
    CodeGen->EmitCode("}");
    CodeGen->tab_count--;
...
//  CodeGen->EmitCode("int search_src_candidate_neighbor(int i, int start, int step)");
//  CodeGen->EmitCode("{");
//  CodeGen->tab_count++;
//  CodeGen->EmitStmt("int c_start = (i % CHUNK_SIZE)+i/(CHUNK_SIZE*THREAD_NUM)*(THREAD_NUM*CHUNK_SIZE)");
//  CodeGen->EmitStmt("int c_end = c_start+(THREAD_NUM-1)*CHUNK_SIZE");
//  CodeGen->EmitCode("for (int c=i+CHUNK_SIZE; c<=c_end; c=c+CHUNK_SIZE) {");
//  CodeGen->tab_count++;
//  CodeGen->EmitStmt("if (addr(i) == addr(c)) { return getThreadID(c,start,step); }");
//  CodeGen->tab_count--;
//  CodeGen->EmitCode("}");
//  CodeGen->EmitStmt("return -1");
//  CodeGen->tab_count--;
//  CodeGen->EmitCode("}");
//  CodeGen->EmitCode("int search_sink_candidate_neighbor(int i, int start, int step)");
//  CodeGen->EmitCode("{");
//  CodeGen->tab_count++;
//  CodeGen->EmitStmt("int c_start = (i % CHUNK_SIZE)+i/(CHUNK_SIZE*THREAD_NUM)*(THREAD_NUM*CHUNK_SIZE)");
//  CodeGen->EmitCode("for (int c=c_start; c<=i; c=c+CHUNK_SIZE) {");
//  CodeGen->tab_count++;
//  CodeGen->EmitStmt("if (addr(i) == addr(c)) { return getThreadID(c,start,step); }");
//  CodeGen->tab_count--;
//  CodeGen->EmitCode("}");
...
void ModelCodeGenWrapperPass::ModelGen(RefTNode *Src, LoopTNode *SrcParallelLoop,
                                         RefTNode *Sink, LoopTNode *SinkParallelLoop,
                                         bool isFakeSrcOrSink) {
  CodeGen->EmitStmt("string reuse_type = \"no-type\"");
  ...
  if (SrcParallelLoop || SinkParallelLoop) {
    // src_has_parallel_induction and sink_has_parallel_induction determines
    // whether we need to search for neighbor accesses. Checking neighbor access
    // is necessary iff both reuse src and sink are inside the parallel loop nests
    //
    // Reference need to search neighbor accesses:
    // A[j][i], A[j], A[i]
    // Ref should be inside a parallel region and the parallel induction var
    // should be at the
    //
    // special case:
    // if the sink access is outside the parallel region, we do not
    // have to find the sink neighbor access
    // example 1:
    // for (i = 0; i < N; i++)
    //    A[i]
    ...
    //    //      A[j]
    //    //    A[i]
    // when there is a reuse between A[j] and A[i], all neighbor of A[j] had
    // already been checked before go to A[i], then we can assume it does not
    // for any reuse with its neighbor
    bool src_has_parallel_induction = false, sink_has_parallel_induction = false,
        src_need_check_neighbor_effect = ((SrcParallelLoop != nullptr) && (SinkParallelLoop != nullptr) && !isFakeSrcOrSink),
        sink_need_check_neighbor_effect = ((SrcParallelLoop != nullptr) && (SinkParallelLoop != nullptr) && !isFakeSrcOrSink);
    if (src_need_check_neighbor_effect) {
      for (unsigned i = 0; i < Src->getSubscripts().size(); i++) {
        Value *Index = Src->getSubscripts()[i];
        if (hasParallelLoopInductionVar(Index)) {
          src_need_check_neighbor_effect =
              (i == Src->getSubscripts().size() - 1);
          src_has_parallel_induction = true;
          break;
        }
      }
    }
    if (sink_need_check_neighbor_effect) {
      for (unsigned i = 0; i < Sink->getSubscripts().size(); i++) {
        Value *Index = Sink->getSubscripts()[i];
        if (hasParallelLoopInductionVar(Index)) {
          sink_need_check_neighbor_effect =
              (i == Sink->getSubscripts().size() - 1);
          sink_has_parallel_induction = true;
          break;
        }
      }
    }
    CodeGen->EmitComment(
        "Src(" + SPSNodeNameTable[Src] +
        ") has Parallel Induction : " + to_string(src_has_parallel_induction));
    CodeGen->EmitComment(
        "Sink(" + SPSNodeNameTable[Sink] +
        ") has Parallel Induction : " + to_string(sink_has_parallel_induction));
    if (SrcParallelLoop) {
      string loop_init_expr = "0", loop_final_expr = "N", loop_step = "1";
      string parallel_loop_trip = EmitLoopTripExpr(SrcParallelLoop);
      string tmp = "";
      TranslateStatus status = SUCCESS;
      tmp = Translator->ValueToStringExpr(SrcParallelLoop->getLoopBound()->InitValue, status);
      if (status == SUCCESS)
        loop_init_expr = tmp;
      tmp = Translator->ValueToStringExpr(SrcParallelLoop->getLoopBound()->FinalValue, status);
      if (status == SUCCESS)
        loop_final_expr = tmp;
      tmp = Translator->ValueToStringExpr(SrcParallelLoop->getLoopBound()->StepValue, status);
      if (status == SUCCESS)
        loop_step = tmp;

      stringstream ss;
      ss << "((" << loop_final_expr << "-" << loop_init_expr << ")/" << loop_step << ")*" << loop_step << "+" << loop_init_expr;
      switch (SrcParallelLoop->getLoopBound()->Predicate) {
      case llvm::CmpInst::ICMP_UGT:
      case llvm::CmpInst::ICMP_SGT:
      case llvm::CmpInst::ICMP_ULT:
      case llvm::CmpInst::ICMP_SLT:
      case llvm::CmpInst::ICMP_NE:
        ss << "-((" + loop_final_expr + "-" + loop_init_expr + ")%" +
              loop_step + "==0)";;
        break;
      default:
        break;
      }
      CodeGen->EmitStmt("bool src_has_full_complete_chunk = ("
                        "hasCompleteChunk("+parallel_loop_trip+") == 0)");
      CodeGen->EmitStmt("int tsrclast = THREAD_NUM-1");
      CodeGen->EmitStmt("int last_src_chunk_pos = CHUNK_SIZE-1");
      CodeGen->EmitStmt("int complement_src_parallel_iteration = 0");
      CodeGen->EmitStmt("int last_src_iteration = " + ss.str());
      ss.str("");
      CodeGen->EmitStmt("int last_src_chunk = getChunkID(last_src_iteration,"+loop_init_expr+","+loop_step+")");
      CodeGen->EmitCode("if (!src_has_full_complete_chunk) {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt("tsrclast = getThreadID(last_src_iteration,"
                        +loop_init_expr+","+loop_step+")");
      CodeGen->EmitStmt("last_src_chunk_pos = getThreadLocalPos(last_src_iteration,"
                        +loop_init_expr+","+loop_step+")");
      CodeGen->EmitStmt("complement_src_parallel_iteration = (THREAD_NUM-tsrclast-1)*CHUNK_SIZE+(CHUNK_SIZE-last_src_chunk_pos-1)");
      CodeGen->EmitDebugInfo("cout << \"last src iteration: \" << last_src_iteration "
                             "<< \" tsrclast: \" << tsrclast "
                             "<< \" last src chunk pos: \" << last_src_chunk_pos "
                             "<< \" last src chunk id: \" << last_src_chunk << endl");
      CodeGen->EmitDebugInfo("cout << \"need to complement \" << complement_src_parallel_iteration << \" iterations to make src loop complete \" << endl");
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
    }
    if (SinkParallelLoop) {
      string loop_init_expr = "0", loop_final_expr = "N", loop_step = "1";
      string parallel_loop_trip = EmitLoopTripExpr(SinkParallelLoop);
      string tmp = "";
      TranslateStatus status = SUCCESS;
      tmp = Translator->ValueToStringExpr(SinkParallelLoop->getLoopBound()->InitValue, status);
      if (status == SUCCESS)
        loop_init_expr = tmp;
      tmp = Translator->ValueToStringExpr(SinkParallelLoop->getLoopBound()->FinalValue, status);
      if (status == SUCCESS)
        loop_final_expr = tmp;
      tmp = Translator->ValueToStringExpr(SinkParallelLoop->getLoopBound()->StepValue, status);
      if (status == SUCCESS)
        loop_step = tmp;
      stringstream ss;
      ss << "((" << loop_final_expr << "-" << loop_init_expr << ")/" << loop_step << ")*" << loop_step << "+" << loop_init_expr;
      switch (SinkParallelLoop->getLoopBound()->Predicate) {
      case llvm::CmpInst::ICMP_UGT:
      case llvm::CmpInst::ICMP_SGT:
      case llvm::CmpInst::ICMP_ULT:
      case llvm::CmpInst::ICMP_SLT:
      ...
      CodeGen->EmitStmt("bool sink_has_full_complete_chunk = ("
                        "hasCompleteChunk("+parallel_loop_trip+") == 0)");
      CodeGen->EmitStmt("int tsinklast = THREAD_NUM-1");
      CodeGen->EmitStmt("int last_sink_chunk_pos = CHUNK_SIZE-1");
      CodeGen->EmitStmt("int complement_sink_parallel_iteration = 0");
      CodeGen->EmitStmt("int last_sink_iteration = " + ss.str());
      CodeGen->EmitStmt("int last_sink_chunk = getChunkID(last_sink_iteration,"+loop_init_expr+","+loop_step+")");
      CodeGen->EmitCode("if (!sink_has_full_complete_chunk) {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt("tsinklast = getThreadID(last_sink_iteration,"
                        +loop_init_expr+","+loop_step+")");
      CodeGen->EmitStmt("last_sink_chunk_pos = getThreadLocalPos(last_sink_iteration,"
                        +loop_init_expr+","+loop_step+")");
      CodeGen->EmitStmt("complement_sink_parallel_iteration = (THREAD_NUM-tsinklast-1)*CHUNK_SIZE+(CHUNK_SIZE-last_sink_chunk_pos-1)");
      CodeGen->EmitDebugInfo("cout << \"last sink iteration: \" << last_sink_iteration "
                             "<< \" tsinklast: \" << tsinklast "
                             "<< \" last sink chunk pos: \" << last_sink_chunk_pos "
                             "<< \" last sink chunk id: \" << last_sink_chunk << endl");
      CodeGen->EmitDebugInfo("cout << \"need to complement \" << complement_sink_parallel_iteration << \" iterations to make sink loop complete \" << endl");
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
    }
    if (SrcParallelLoop && SinkParallelLoop) {
      if (SrcParallelLoop == SinkParallelLoop) {
        CodeGen->EmitComment(SPSNodeNameTable[Src] + " and " +
                             SPSNodeNameTable[Sink] + " is in same parallel loop nests");
      } else {
        CodeGen->EmitComment(SPSNodeNameTable[Src] + " and " +
                             SPSNodeNameTable[Sink] + " is in different parallel loop nests");
      }
    } else if (SrcParallelLoop) {
      CodeGen->EmitComment(SPSNodeNameTable[Src] + " is in parallel loop nest and " +
                           SPSNodeNameTable[Sink] + " is in sequential loop nests");
    } else if (SinkParallelLoop) {
      CodeGen->EmitComment(SPSNodeNameTable[Src] + " is in sequential loop nest and " +
                           SPSNodeNameTable[Sink] + " is in parallel loop nests");
    } else {
      CodeGen->EmitComment(SPSNodeNameTable[Src] + " and " +
                           SPSNodeNameTable[Sink] + " is in sequential loop nests");
    }
    if (src_need_check_neighbor_effect)
      CodeGen->EmitComment("Need to check src neighbor effect for " +
                           SPSNodeNameTable[Src]);
    if (sink_need_check_neighbor_effect)
      CodeGen->EmitComment("Need to check sink neighbor effect for " +
                           SPSNodeNameTable[Sink]);
    if (src_need_check_neighbor_effect || sink_need_check_neighbor_effect) {
      CodeGen->EmitStmt("int c_start = 0");
      CodeGen->EmitStmt("int neighbor_tid = -1");
    }
    // before the model, we set the parallel_reuse to reuse
    CodeGen->EmitStmt("int parallel_reuse = (int)reuse");

    // If src and sink all in parallel region
    // reuse = reuse
    // If src is in parallel region
    // reuse = parallel_count
    // if find_src_neighbor_reuse
    //   parallel_reuse = neighbor_reuse
    // else
    //   parallel_reuse = sequential_reuse + f(reuse)
    // If sink is in parallel region
    // reuse = parallel_count
    // parallel_reuse = f(reuse) + sequential_reuse + delta

    // src parallel induction variable access
    // need to derive the location of the parallel induction variable
    TranslateStatus status = SUCCESS;

    string loop_init_expr = "0", loop_step = "1", loop_final_expr = "";
    string tmp = "", loop_trip = "";
    if (SrcParallelLoop) {
      tmp = Translator->ValueToStringExpr(
          SrcParallelLoop->getLoopBound()->InitValue, status);
      if (status == SUCCESS) {
        loop_init_expr = tmp;
      }
      tmp = Translator->ValueToStringExpr(
          SrcParallelLoop->getLoopBound()->StepValue, status);
      if (status == SUCCESS)
        loop_step = tmp;
      AccPath pathToTreeRoot;
      QueryEngine->GetPath(SrcParallelLoop, nullptr, pathToTreeRoot);
      CodeGen->EmitComment("(src->ivs[" + to_string(pathToTreeRoot.size()) +
                           "]-" + loop_init_expr + ")");
      CodeGen->EmitStmt("bool find_src_neighbor_reuse = false");
      CodeGen->EmitStmt("int tsrc = getThreadID("
                        "src->ivs[" +
                        to_string(pathToTreeRoot.size()) + "]," +
                        loop_init_expr + "," + loop_step + ")");
      CodeGen->EmitStmt("int chunk_src = getChunkID("
                        "src->ivs[" +
                        to_string(pathToTreeRoot.size()) + "]," +
                        loop_init_expr + "," + loop_step + ")");
      CodeGen->EmitStmt("int local_pos_src = getThreadLocalPos("
                        "src->ivs[" +
                        to_string(pathToTreeRoot.size()) + "]," +
                        loop_init_expr + "," + loop_step + ")");
      // generate code to find neighbor effect
      if (src_need_check_neighbor_effect) {
#if 0
        CodeGen->EmitStmt(
            "c_start = src->ivs[" + to_string(pathToTreeRoot.size()) + "]" +
            "%CHUNK_SIZE+src->ivs[" + to_string(pathToTreeRoot.size()) +
            "]/(CHUNK_SIZE*THREAD_NUM)*THREAD_NUM*CHUNK_SIZE");
        CodeGen->EmitStmt("int c_end = c_start+(THREAD_NUM-1)*CHUNK_SIZE");
        CodeGen->EmitCode("for (int c=src->ivs[" +
                          to_string(pathToTreeRoot.size()) +
                          "]+CHUNK_SIZE; c<=c_end; c+=CHUNK_SIZE) {");
#endif
        CodeGen->EmitCode("for (int t = tsrc+1; t <= tsrclast; t++) {");
        CodeGen->tab_count++;
        CodeGen->EmitStmt("int c = src->ivs["+to_string(pathToTreeRoot.size())+"]+CHUNK_SIZE*(t-tsrc)*"+loop_step);
        CodeGen->EmitCode("if (c > last_src_iteration) { break; }");
#if 0
        CodeGen->EmitStmt("string neighbor_addr = \"\"");
        for (auto Index : Src->getSubscripts()) {
          tmp = ReplaceInductionVarExprByType(Index, MODEL, status);
          if (Index != Src->getSubscripts().back())
            CodeGen->EmitStmt("neighbor_addr += (to_string(" + tmp +
                              ")+\"_\")");
          else
            CodeGen->EmitStmt("neighbor_addr += to_string(" + tmp + ")");
        }
#endif
        stringstream ss;
        ss << "long neighbor_addr = GetAddress_" << SPSNodeNameTable[Src] << "(";
        for (auto Index : Src->getSubscripts()) {
          tmp = ReplaceInductionVarExprByType(Index, MODEL, status);
          if (Index != Src->getSubscripts().back())
            ss << tmp << ",";
        ...
        ss.str("");
        CodeGen->EmitCode("if (neighbor_addr == addr) {");
        CodeGen->tab_count++;
        CodeGen->EmitStmt("neighbor_tid = t");
        CodeGen->EmitStmt("reuse_type = \"src-neighbor\"");
        CodeGen->EmitStmt("break");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->EmitCode("if (neighbor_tid >= 0) {");
        CodeGen->tab_count++;
        CodeGen->EmitComment("Find neighbor src access");
        CodeGen->EmitStmt("parallel_reuse = (neighbor_tid - tsrc)");
        CodeGen->EmitStmt("find_src_neighbor_reuse = true");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->EmitCode("if (!find_src_neighbor_reuse) {");
        CodeGen->tab_count++;
      }
    }
#if 0
    } else {
      CodeGen->EmitComment("Assume the first access in the parallel region forms a reuse with the sink");
      CodeGen->EmitStmt("int tsrc = 0");
      CodeGen->EmitStmt("int chunk_src = 0");
      CodeGen->EmitStmt("int local_pos_src = 0");
    }
#endif
    // sink parallel induction variable access
    if (SinkParallelLoop) {
      string parallel_loop_induction = Translator->ValueToStringExpr(
          SinkParallelLoop->getInductionPhi(), status);
      CodeGen->EmitComment("(" + parallel_loop_induction + "-" +
                           loop_init_expr + ")");
      tmp = Translator->ValueToStringExpr(
          SinkParallelLoop->getLoopBound()->InitValue, status);
      if (status == SUCCESS) {
        loop_init_expr = tmp;
      }
      tmp = Translator->ValueToStringExpr(
          SinkParallelLoop->getLoopBound()->StepValue, status);
      if (status == SUCCESS)
        loop_step = tmp;
      CodeGen->EmitStmt("int tsink = getThreadID(" + parallel_loop_induction +
                        "," + loop_init_expr + "," + loop_step + ")");
      CodeGen->EmitStmt("int chunk_sink = getChunkID(" +
                        parallel_loop_induction + "," + loop_init_expr + "," +
                        loop_step + ")");
      CodeGen->EmitStmt("int local_pos_sink = getThreadLocalPos(" +
                        parallel_loop_induction + "," + loop_init_expr + "," +
                        loop_step + ")");
      if (sink_need_check_neighbor_effect) {
        CodeGen->EmitStmt("int delta = 0");
        CodeGen->EmitStmt("neighbor_tid = -1");
#if 0
        CodeGen->EmitStmt("c_start = " + parallel_loop_induction +
                          "%CHUNK_SIZE+" + parallel_loop_induction +
                          "/(CHUNK_SIZE*THREAD_NUM)*THREAD_NUM*CHUNK_SIZE");
        CodeGen->EmitCode("for (int c=c_start; c<=" + parallel_loop_induction +
                          "; c+=CHUNK_SIZE) {");
#endif
        CodeGen->EmitCode("for (int t = 0; t < tsink; t++) {");
        CodeGen->tab_count++;
        CodeGen->EmitStmt("int c = "+parallel_loop_induction+"-(tsink-t)*CHUNK_SIZE*"+loop_step);
        CodeGen->EmitCode("if (c > last_sink_iteration) { break; }");
#if 0
        CodeGen->EmitStmt("string neighbor_addr = \"\"");
        for (auto Index : Sink->getSubscripts()) {
          tmp = Translator->ValueToStringExpr(Index, status);
          ReplaceSubstringWith(tmp, parallel_loop_induction, "c");
          if (Index != Sink->getSubscripts().back())
            CodeGen->EmitStmt("neighbor_addr += (to_string(" + tmp +
                              ")+\"_\")");
          else
            CodeGen->EmitStmt("neighbor_addr += to_string(" + tmp + ")");
...
#endif
        stringstream ss;
        ss << "uint64_t neighbor_addr = GetAddress_" << SPSNodeNameTable[Src] << "(";
        for (auto Index : Sink->getSubscripts()) {
          tmp = Translator->ValueToStringExpr(Index, status);
          ReplaceSubstringWith(tmp, parallel_loop_induction, "c");
          if (Index != Sink->getSubscripts().back())
        ...
        CodeGen->EmitCode("if (neighbor_tid >= 0) {");
        CodeGen->tab_count++;
        CodeGen->EmitComment("Find neighbor sink access");
        CodeGen->EmitStmt("delta = neighbor_tid-tsink");
        CodeGen->EmitStmt("reuse_type = \"sink-neighbor\"");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
      }
    }

    // src is in sequential part, sink is in parallel part
    if (!SrcParallelLoop && SinkParallelLoop) {
//      CodeGen->EmitStmt("sequential_part = sequential_count - SequentialLAT[src]");
    }
#if 0
    } else {
      if (status == SUCCESS) {
      ...
      if (status == SUCCESS)
        loop_step = tmp;
      tmp = Translator->ValueToStringExpr(
          SrcParallelLoop->getLoopBound()->FinalValue, status);
      if (status == SUCCESS)
        loop_final_expr = tmp;
      switch(SrcParallelLoop->getLoopBound()->Predicate) {
      case llvm::CmpInst::ICMP_UGE:
      ...
      case llvm::CmpInst::ICMP_SGE:
      case llvm::CmpInst::ICMP_SLE:
        loop_trip = "(" + loop_final_expr + "-" + loop_init_expr + "+1)" + "/" + loop_step;
        break;
      default:
        loop_trip = "(" + loop_final_expr + "-" + loop_init_expr + ")" + "/" + loop_step;
        break;
      }
      CodeGen->EmitComment("Assume the last access in the parallel region forms a reuse with the src");
      CodeGen->EmitStmt("int tsink = getThreadID("+loop_final_expr+","+loop_init_expr+","+loop_step+")");
      CodeGen->EmitStmt("int chunk_sink = getChunkNum("+loop_trip+")");
      CodeGen->EmitStmt("int local_pos_sink = getThreadLocalPos("+loop_final_expr+","+loop_init_expr+","+loop_step+")");
    }
#endif
    AccPath path;
    QueryEngine->GetPath(Sink, nullptr, path);
    auto pathIter = path.rbegin();
    stringstream iterationss;
    iterationss << "\"" << SPSNodeNameTable[Sink] << " (\" << ";
    while (pathIter != path.rend()) {
      ...
      string nest_loop_induction = ReplaceInductionVarExprByType(
          NestLoop->getInductionPhi(), SAMPLE, status);
      iterationss << nest_loop_induction << " << ";
      if (pathIter + 1 != path.rend())
        iterationss << "\",\" << ";
      pathIter++;
    }
    CodeGen->EmitDebugInfo("cout << \"[\" << tsrc << \"] Src ChunkID \" << chunk_src << \" \" << src->toString() << endl");
    CodeGen->EmitDebugInfo(
        "cout << \"[\" << tsink << \"] Sink ChunkID \" << chunk_sink << \" \" << " +
        iterationss.str() + "\")\" << endl");
    // interchunk reuse, need to compute middle access count
    CodeGen->EmitStmt("int dT = tsink - tsrc");
    string ParallelSrcLoopTripExpr = "0", ParallelSinkLoopTripExpr = "0";
    if (SrcParallelLoop) {
      ParallelSrcLoopTripExpr =
          EmitOneParallelIterationTripFormula(SrcParallelLoop);
    } else {
      // Get the first access of the SinkParallelLoop
      ParallelSrcLoopTripExpr = EmitOneParallelIterationTripFormula(SinkParallelLoop);
    }
    if (SinkParallelLoop) {
      ParallelSinkLoopTripExpr =
          EmitOneParallelIterationTripFormula(SinkParallelLoop);
    } else {
      ParallelSinkLoopTripExpr =
          EmitOneParallelIterationTripFormula(SrcParallelLoop);
    }
    ...
    if ((SrcParallelLoop && SinkParallelLoop) && SrcParallelLoop != SinkParallelLoop) {
#if 0
      CodeGen->EmitComment(SrcParallelLoop->getLoopStringExpr() +
                         " iteration: " + ParallelSrcLoopTripExpr);
    CodeGen->EmitComment(SinkParallelLoop->getLoopStringExpr() +
                         " iteration: " + ParallelSinkLoopTripExpr);
    CodeGen->EmitComment("\"Middle access count \"" + ParallelSrcLoopTripExpr + " << endl");
#endif
      string parallel_loop_trip = EmitLoopTripExpr(SrcParallelLoop);
      CodeGen->EmitCode("if (hasCompleteChunk(" + parallel_loop_trip +
                        ") == 0) {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt(
          "middle_accesses += ((getChunkNum(" + parallel_loop_trip +
          ")-chunk_src-1)*(CHUNK_SIZE*THREAD_NUM))*" +
          ParallelSrcLoopTripExpr + "+chunk_sink*CHUNK_SIZE*THREAD_NUM*" +
          ParallelSinkLoopTripExpr);
      CodeGen->tab_count--;
      CodeGen->EmitCode("} else if (chunk_src == getChunkNum("+parallel_loop_trip+")-1) {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt(
          "middle_accesses += chunk_sink*CHUNK_SIZE*THREAD_NUM*" + ParallelSinkLoopTripExpr);
      CodeGen->tab_count--;
      CodeGen->EmitCode("} else {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt(
          "middle_accesses += ((getChunkNum(" + parallel_loop_trip +
          ")-chunk_src-2)*(CHUNK_SIZE*THREAD_NUM)+(" + parallel_loop_trip +
          "%(THREAD_NUM*CHUNK_SIZE)))*" + ParallelSrcLoopTripExpr +
          "+chunk_sink*CHUNK_SIZE*THREAD_NUM*" + ParallelSinkLoopTripExpr);
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
      CodeGen->EmitStmt("middle_accesses += " +
                        MiddleAccessCount(SrcParallelLoop, SinkParallelLoop));
      CodeGen->EmitDebugInfo("cout << \"Source Loop Iteration \" << " +
                             ParallelSrcLoopTripExpr + " << endl");
      CodeGen->EmitDebugInfo("cout << \"Sink Loop Iteration \" << " +
                             ParallelSinkLoopTripExpr + " << endl");
      CodeGen->EmitDebugInfo("cout << \"Middle accesses \" << middle_accesses << endl");
      CodeGen->EmitComment("interchunk effect");

      if (sink_need_check_neighbor_effect)
        CodeGen->EmitStmt(
            "parallel_reuse = reuse*THREAD_NUM-CHUNK_SIZE*THREAD_NUM"
            "*(" +
            ParallelSrcLoopTripExpr + "*(THREAD_NUM-tsrc)+" +
            ParallelSinkLoopTripExpr + "*tsink)+CHUNK_SIZE*THREAD_NUM*" +
            ParallelSrcLoopTripExpr + "-(THREAD_NUM-1)*middle_accesses+dT+delta");
      else if (isFakeSrcOrSink)
        CodeGen->EmitStmt(
            "parallel_reuse = parallel_count*THREAD_NUM-CHUNK_SIZE*THREAD_NUM"
            "*(" +
            ParallelSrcLoopTripExpr + "*(THREAD_NUM-tsrc)+" +
            ParallelSinkLoopTripExpr + "*tsink)+CHUNK_SIZE*THREAD_NUM*" +
            ParallelSrcLoopTripExpr + "-(THREAD_NUM-1)*middle_accesses+dT");
      else
        CodeGen->EmitStmt(
            "parallel_reuse = reuse*THREAD_NUM-CHUNK_SIZE*THREAD_NUM"
            "*(" +
            ...
            ParallelSinkLoopTripExpr + "*tsink)+CHUNK_SIZE*THREAD_NUM*" +
            ParallelSrcLoopTripExpr + "-(THREAD_NUM-1)*middle_accesses+dT");
      CodeGen->EmitStmt("reuse_type = \"inter-chunk\"");
      CodeGen->EmitStmt("goto PARALLEL_REUSE_UPDATE_" + SPSNodeNameTable[Sink]);
    } else if ((SrcParallelLoop && SinkParallelLoop) && SrcParallelLoop == SinkParallelLoop) {
      // if src and sink in different chunks, it is also a interchunk reuse,
      // need to compute middle access count
      CodeGen->EmitCode("if (chunk_src != chunk_sink) {");
      CodeGen->tab_count++;
#if 0
      CodeGen->EmitComment(SrcParallelLoop->getLoopStringExpr() +
                         " iteration: " + ParallelSrcLoopTripExpr);
    CodeGen->EmitComment(SinkParallelLoop->getLoopStringExpr() +
                         " iteration: " + ParallelSinkLoopTripExpr);
    CodeGen->EmitComment("Middle access count " + MiddleAccess);
#endif
      CodeGen->EmitDebugInfo("cout << \"Source Loop Iteration \" << " +
                             ParallelSrcLoopTripExpr + " << endl");
      CodeGen->EmitDebugInfo("cout << \"Sink Loop Iteration \" << " +
                             ParallelSinkLoopTripExpr + " << endl");
      CodeGen->EmitDebugInfo("cout << \"Middle accesses \" << middle_accesses"
                             " << endl");
      CodeGen->EmitComment("interchunk effect");
      if (sink_need_check_neighbor_effect)
        CodeGen->EmitStmt(
        ...
        CodeGen->EmitStmt(
            ...
            ParallelSinkLoopTripExpr + "*tsink)+CHUNK_SIZE*THREAD_NUM*" +
            ParallelSrcLoopTripExpr + "-(THREAD_NUM-1)*middle_accesses+dT");
      // here chunk_src != chunk_sink, so only check whether the sink chunk
      // is the none-complete chunk
      CodeGen->EmitStmt("reuse_type = \"inter-chunk\"");
      CodeGen->EmitStmt("goto PARALLEL_REUSE_UPDATE_" + SPSNodeNameTable[Sink]);
      CodeGen->tab_count--;
      // fold/scale effect
      CodeGen->EmitCode("} else if (tsrc != tsink) {");
      CodeGen->tab_count++;
      CodeGen->EmitComment("fold effect, check if src-sink or sink-src order");
      CodeGen->EmitCode("if (local_pos_src <= local_pos_sink) {");
      CodeGen->tab_count++;
      if (sink_need_check_neighbor_effect)
        CodeGen->EmitStmt("parallel_reuse = reuse*THREAD_NUM-CHUNK_SIZE*" +
                          ParallelSrcLoopTripExpr +
                          "*THREAD_NUM*dT+abs(dT)+delta");
      else if (isFakeSrcOrSink)
        CodeGen->EmitStmt("parallel_reuse = parallel_count*THREAD_NUM-CHUNK_SIZE*" +
                          ParallelSrcLoopTripExpr + "*THREAD_NUM*dT+abs(dT)");
      else
        CodeGen->EmitStmt("parallel_reuse = reuse*THREAD_NUM-CHUNK_SIZE*" +
                          ParallelSrcLoopTripExpr + "*THREAD_NUM*dT+abs(dT)");
      CodeGen->EmitStmt("reuse_type = \"src-sink-fold\"");
      CodeGen->EmitStmt("goto PARALLEL_REUSE_UPDATE_" + SPSNodeNameTable[Sink]);
      CodeGen->tab_count--;
      CodeGen->EmitCode("} else {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt("reuse_type = \"sink-src-fold\"");
      CodeGen->EmitStmt("parallel_reuse = -1");
      CodeGen->EmitStmt("goto PARALLEL_REUSE_UPDATE_" + SPSNodeNameTable[Sink]);
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
      CodeGen->tab_count--;
      CodeGen->EmitCode("} else {");
      CodeGen->tab_count++;
      CodeGen->EmitComment("scale effect");
      if (sink_need_check_neighbor_effect)
        CodeGen->EmitStmt("parallel_reuse = reuse*THREAD_NUM+delta");
      else if (isFakeSrcOrSink)
        CodeGen->EmitStmt("parallel_reuse = parallel_count*THREAD_NUM");
      else
        CodeGen->EmitStmt("parallel_reuse = reuse*THREAD_NUM");
      CodeGen->EmitStmt("reuse_type = \"scale\"");
      CodeGen->EmitStmt("goto PARALLEL_REUSE_UPDATE_" + SPSNodeNameTable[Sink]);
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
    }
    if (src_need_check_neighbor_effect) {
      CodeGen->tab_count--;
      CodeGen->EmitCode("} // end of if (!find_src_neighbor_reuse) ");
    }
  } else {
...
string ModelCodeGenWrapperPass::MiddleAccessCount(LoopTNode *SrcLoop, LoopTNode *SinkLoop)
{
  if (SrcLoop == SinkLoop)
    return "0";
  SmallVector<SPSTNode *, 4> ImmediateDominator;
  QueryEngine->GetImmediateCommonLoopDominator(SrcLoop, SinkLoop, false, ImmediateDominator);
  stringstream ss;
  bool find_src_loop = false;
  unsigned middle_reference_cnt = 0;
#if 0
  TranslateStatus status = SUCCESS;
  string loop_init_expr = "0", loop_final_expr = "", loop_step = "1", loop_trip = "";
  string tmp = Translator->ValueToStringExpr(
      SrcLoop->getLoopBound()->InitValue, status);
  if (status == SUCCESS) {
    loop_init_expr = tmp;
  }
  tmp = Translator->ValueToStringExpr(
      SrcLoop->getLoopBound()->StepValue, status);
  if (status == SUCCESS)
    loop_step = tmp;
  tmp = Translator->ValueToStringExpr(
      SrcLoop->getLoopBound()->FinalValue, status);
  if (status == SUCCESS)
    loop_final_expr = tmp;
  switch(SrcLoop->getLoopBound()->Predicate) {
  case llvm::CmpInst::ICMP_UGE:
  case llvm::CmpInst::ICMP_ULE:
  case llvm::CmpInst::ICMP_SGE:
  case llvm::CmpInst::ICMP_SLE:
  ...
  AccPath pathToTreeRoot;
  QueryEngine->GetPath(SrcLoop, nullptr, pathToTreeRoot);
  string lsrc_expr = "((getChunkNum("+loop_trip+")"
                      +"-getChunkID(src->ivs[" + to_string(pathToTreeRoot.size()) + "],"
                      +loop_init_expr+","+loop_step+")-1)*"
                      +EmitOneParallelIterationTripFormula(SrcLoop)+")";

  string parallel_loop_induction =
      Translator->ValueToStringExpr(SinkLoop->getInductionPhi(), status);
  tmp = Translator->ValueToStringExpr(
      SinkLoop->getLoopBound()->InitValue, status);
  if (status == SUCCESS) {
    loop_init_expr = tmp;
  }
  tmp = Translator->ValueToStringExpr(
      SinkLoop->getLoopBound()->StepValue, status);
  if (status == SUCCESS)
    loop_step = tmp;
  string lsink_expr = "(getChunkID("+parallel_loop_induction+","
      +loop_init_expr+","+loop_step+")*"
      +EmitOneParallelIterationTripFormula(SinkLoop)+")";
#endif

  ss << "(";
  if (ImmediateDominator.empty()) {
    // SrcLoop and SinkLoop are both top level parallel loop
    for (auto TopNode : getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>().TreeRoot->neighbors) {
      if (LoopTNode *Loop = dynamic_cast<LoopTNode *>(TopNode)) {
        if (Loop == SrcLoop) {
          find_src_loop = true;
          continue;
        }
      ...
      if (RefTNode *Ref = dynamic_cast<RefTNode *>(TopNode)) {
        if (find_src_loop)
          middle_reference_cnt += 1;
      }
    }
  } else {
    for (auto TopNode : ImmediateDominator.front()->neighbors) {
      if (LoopTNode *Loop = dynamic_cast<LoopTNode *>(TopNode)) {
        if (Loop == SrcLoop) {
          find_src_loop = true;
          continue;
  ...
  ss << middle_reference_cnt << ")";
#if 0
  // now we compute the unvisited chunks in SrcLoop and SinkLoop
  ss << "+" << lsrc_expr << "+" << lsink_expr;
  CodeGen->EmitDebugInfo("cout << \"src loop chunks \" << " + lsrc_expr + " << endl");
  CodeGen->EmitDebugInfo("cout << \"sink loop chunks \" << " + lsrc_expr + " << endl");
#endif
  return ss.str();
}


void ModelCodeGenWrapperPass::HeaderGen()
{
...
void ModelCodeGenWrapperPass::RefAddressFuncGen()
{
...
void ModelCodeGenWrapperPass::MainFuncGen()
{
  ...
  if ((EnableSampling && (SamplingMethod == RANDOM_START)) || EnablePerReference) {
    ...
    for (auto node : SPSNodeNameTable) {
      if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
        // top level reference will not be analyzed, we should skip those sampler
        // function call
        // top level reference does not have dominator
...
//          CodeGen->EmitStmt("cout << \""+node.second+"\" << endl");
    ...
    if (EnableParallelOpt) {
      ...
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          ...
          CodeGen->EmitStmt("t_sampler_"+node.second+".join()");
        }
      }
      CodeGen->EmitComment("Merge the histogram found by each reference");
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          CodeGen->EmitCode("for (auto pair : histogram_"+node.second+") {");
          CodeGen->tab_count++;
          CodeGen->EmitStmt("histogram[pair.first] += pair.second");
          CodeGen->tab_count--;
          CodeGen->EmitCode("}");
        }
      }
...
//      CodeGen->EmitStmt("cout << \"Begin Loop \" << loop_cnt << \"\\n\"");
      CodeGen->EmitFunctionCall("pluss_print_mrc", {});
//      CodeGen->EmitStmt("cout << \"End Loop \" << loop_cnt << \"\\n\"");
...
void ModelCodeGenWrapperPass::SamplerBodyGen(bool isPerReference)
{
  ...
  if (!isPerReference) {
    ...
    for (; topiter != getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>()
        .TreeRoot->neighbors.end();
           topiter++) {
      SamplerBodyGenImpl(*topiter);
    }
    CodeGen->EmitHashMacro("#if defined(DEBUG)");
    if (InterleavingTechnique == UNIFORM_INTERLEAVING) {
      CodeGen->EmitCode(
          "for (auto entry : LATIterMap) { delete entry.second; }");
    } else {
      CodeGen->EmitCode("for (int t = 0; t < THREAD_NUM; t++) {");
      CodeGen->tab_count++;
      CodeGen->EmitCode(
    ...
    CodeGen->EmitStmt("LATIterMap.clear()");
    if (InterleavingTechnique == 0) {
      CodeGen->EmitStmt("max_iteration_count = count");
    } else {
      CodeGen->EmitStmt("max_iteration_count = accumulate(count.begin(), count.end(), 0)");
    }
...
//                                    induction_expr + ") + \"-\")");
                ...
                CodeGen->EmitStmt("if (!LAT.empty()) { LAT.clear(); }");
        ...
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          ...
          CodeGen->EmitCode("}");
//          if (!EnableParallelOpt)
//            CodeGen->EmitFunctionCall("pluss_reset", {});
          if (InterleavingTechnique == 0) {
            CodeGen->EmitCode("if (!LATSampleIterMap.empty()) {");
            CodeGen->tab_count++;
            CodeGen->EmitCode("for (auto entry : LATSampleIterMap) {");
            CodeGen->tab_count++;
            CodeGen->EmitStmt("delete entry.second");
            CodeGen->tab_count--;
            CodeGen->EmitCode("}");
            ...
            CodeGen->tab_count--;
            CodeGen->EmitCode("}");
            CodeGen->EmitDebugInfo("cout << \"Sequential Src Sequential Sink: \" << sequential_src_sequential_sink_cnt << endl");
            CodeGen->EmitDebugInfo("cout << \"Sequential Src Parallel Sink: \" << sequential_src_parallel_sink_cnt << endl");
            CodeGen->EmitDebugInfo("cout << \"Parallel Src Sequential Sink: \" << parallel_src_sequential_sink_cnt << endl");
            CodeGen->EmitDebugInfo("cout << \"Parallel Src Parallel Sink: \" << parallel_src_parallel_sink_cnt << endl");
            CodeGen->EmitStmt(
...
//            CodeGen->EmitStmt("no_share_intra_thread_RI.clear()");
          ...
          CodeGen->EmitStmt("LATSampleIterMap.clear()");
          if (InterleavingTechnique == 0) {
            CodeGen->EmitStmt("iteration_traversed_map[\"" + ref.second +
                              "\"] = count");
          } else {
//            CodeGen->EmitFunctionCall("pluss_timer_start",{});
            CodeGen->EmitFunctionCall("no_share_distribute", {"no_share_intra_thread_RI"});
            CodeGen->EmitFunctionCall("share_distribute", {"share_intra_thread_RI"});
//            CodeGen->EmitFunctionCall("pluss_timer_stop",{});
//            CodeGen->EmitStmt("model_time += pluss_timer_return()");
//            CodeGen->EmitFunctionCall("pluss_AET", {});
//            CodeGen->EmitFunctionCall("pluss_print_histogram", {});
...
void ModelCodeGenWrapperPass::SamplerBodyGenImpl(SPSTNode *Root)
{
...
void ModelCodeGenWrapperPass::LoopIterUpdateGen(AccGraph *G,
                                                  AccGraphEdge *E,
                                                  LoopTNode* IDomLoop,
...
void ModelCodeGenWrapperPass::ParallelSamplerBodyGenImpl(SPSTNode *Root,
                                                           AccGraph *G,
                                                           Interleaving type)
{
  ...
  LLVM_DEBUG(
      while (GraphNodeIter != NodesInGraph.end()) {
        dbgs() << (*GraphNodeIter)->getName() << "\n";
        GraphNodeIter++;
      }
  ...
  while (true) {
    ...
    if (GraphNodeIter == NodesInGraph.begin()) {
      ...
      CodeGen->EmitCode("while(true) {");
      if (type == RANDOM_INTERLEAVING)
        CodeGen->EmitLabel("CHUNK_ASSIGNMENT_LOOP_" + to_string(ParallelLoopNode->getLoopID()));
      CodeGen->tab_count++;
      CodeGen->EmitCode("if (!idle_threads.empty() && dispatcher.hasNextChunk("
      ...
      if (scheduling == DYNAMIC) {
        ...
        CodeGen->EmitDebugInfo("cout << tid_to_run << \" has no available chunks, will wait for other threads.\" << endl;");
        CodeGen->EmitStmt("continue");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
      }
...
//      errs() << "#endif\n";
      if (scheduling == DYNAMIC)
        ...
        CodeGen->EmitStmt("Chunk c = dispatcher.getNextStaticChunk(tid_to_run)");
      ...
      AccPath pathToTreeRoot;
      // backtrack the tree from the currentNode and push all met
      // loopnode inside the path
      G->QueryEngine->GetPath(currentNode, nullptr, pathToTreeRoot);
...
//        LLVM_DEBUG(dbgs() << LoopInPath->getLoopStringExpr() << "\n");
      ...
      // if the loop is outside the parallel loop, we do not put it in the progress
      // iteration vector. They can be viewed as a constant
      // i.e.
      // for (c0 = 0; c0 < n; c0 ++)
      // #pragma omp
      //   for (c1 = 0; c1 < n; c1++)
      ...
      while (path_riter != pathToTreeRoot.rend()) {
        ...
        if (LoopInPath == ParallelLoopNode) { // this is the parallel loop
          ...
          codess << "parallel_iteration_vector.push_back("
                 << Translator->ValueToStringExpr(LoopInPath->getInductionPhi(), status)
                 << ")";
          CodeGen->EmitStmt(codess.str());
          codess.str("");
        } else { // this is the loop inside the parallel loop
          codess << "parallel_iteration_vector.push_back(" << ReplaceInductionVarExprByType(
              LoopInPath->getLoopBound()->InitValue, OMP_PARALLEL_INIT, status)
                 << " )";
          CodeGen->EmitStmt(codess.str());
          codess.str("");
        }
      ...
      if (scheduling == DYNAMIC)
        ...
        CodeGen->EmitStmt("idle_threads_iterator = idle_threads.erase("
                          "idle_threads_iterator)");
      CodeGen->EmitDebugInfo("cout << \"Move \" << tid_to_run << \" to worker_threads\" << endl");
      ...
      if (type == UNIFORM_INTERLEAVING) {
        ...
        CodeGen->EmitStmt("auto worker_thread_iterator = worker_threads.begin()");
        CodeGen->EmitCode(
            "while (worker_thread_iterator != worker_threads.end()) {");
        CodeGen->tab_count++;
        CodeGen->EmitStmt("tid_to_run = *worker_thread_iterator");
...
//      errs() << "#endif\n";
    ...
    if (BranchTNode *Branch = dynamic_cast<BranchTNode *>(currentNode)) {
      ...
      if (!Branch->neighbors[1]->neighbors.empty()) {
        ...
        if (Branch->neighbors[0]->neighbors.empty()) {
          ...
          codess << "progress[tid_to_run]->increment(\""
                 << SPSNodeNameTable[FirstAccesses[0]] << "\")";
          CodeGen->EmitStmt(codess.str());
          codess.str("");
        } else {
          CodeGen->EmitCode("} else {");
      ...
      if (!params.empty()) {
        if (EnableSampling && SamplingMethod == SEQUENTIAL_START) {
          CodeGen->EmitComment("choose a number between 1 and 100");
          string threshold = to_string(SamplingRatio);
          string range = "100";
          if (SamplingRatio < 1) {
            double tmp = SamplingRatio;
            int range_tmp = 100;
            while (tmp < 1.0) {
              tmp *= 10.0;
              range_tmp *= 10;
            }
            threshold = to_string(int(tmp));
            range = to_string(int(range_tmp));
          }
          CodeGen->EmitStmt("int enable = rand() % "+ range +" + 1");
          CodeGen->EmitCode("if (enable <= " + threshold + ") {");
          CodeGen->tab_count++;
        }
//        CodeGen->EmitStmt("subscripts = " + params[1]);
...
//          codess << "subscripts[" << i << "]";
        ...
        AccPath path;
        QueryEngine->GetPath(RefNode, nullptr, path);
        codess << "access = new Iteration(\"" << SPSNodeNameTable[RefNode] << "\", {";
        auto pathIter = path.rbegin();
        TranslateStatus status = SUCCESS;
        while (pathIter != path.rend()) {
          LoopTNode *NestLoop = dynamic_cast<LoopTNode *>(*pathIter);
          assert(NestLoop && "All node in a path should be an instance of LoopTNode");
          string nest_loop_induction = ReplaceInductionVarExprByType(NestLoop->getInductionPhi(), OMP_PARALLEL, status);
        ...
        codess << "})";
        CodeGen->EmitDebugInfo(codess.str());
        codess.str("");
        if (InterleavingTechnique == 0) {
          CodeGen->EmitCode("if (LAT_" + RefNode->getArrayNameString() +
                            ".find(addr) != LAT_" +
          ...
          CodeGen->EmitCode("if (LAT_" + RefNode->getArrayNameString() +
                            "[tid_to_run].find(addr) != LAT_" +
                            RefNode->getArrayNameString() + "[tid_to_run].end()) {");
        }
        CodeGen->tab_count++;
        ...
        pathIter = PathToParallelLoop.rbegin();
        ...
        if (InterleavingTechnique == 0) {
          ...
          CodeGen->EmitHashMacro("#if defined(DEBUG)");
//          CodeGen->EmitCode("if (reuse > 0.9*(" + EmitOneParallelIterationTripFormula(QueryEngine->GetParallelLoopDominator(RefNode)) + ")) {");
          CodeGen->EmitStmt("Iteration *src = LATIterMap[LAT_"+RefNode->getArrayNameString()+"[addr]]");
          CodeGen->EmitStmt("cout << \"[\" << reuse << \"] \" << src->toString() << \" -> \" << access->toString() << endl");
          CodeGen->EmitHashMacro("#endif");
        } else {
          ...
          CodeGen->EmitHashMacro("#if defined(DEBUG)");
//          CodeGen->EmitCode("if (reuse > 0.9*(" + EmitOneParallelIterationTripFormula(QueryEngine->GetParallelLoopDominator(RefNode)) + ")) {");
          CodeGen->EmitCode("if (reuse >= 512) {");
          CodeGen->tab_count++;
          CodeGen->EmitStmt("Iteration *src = LATIterMap[tid_to_run][LAT_"+RefNode->getArrayNameString()+"[tid_to_run][addr]]");
          CodeGen->EmitStmt("cout << \"[\" << reuse << \"] \" << src->toString() << \" -> \" << access->toString() << endl");
          CodeGen->tab_count--;
          CodeGen->EmitCode("}");
          CodeGen->EmitHashMacro("#endif");
        ...
        if (InterleavingTechnique == 0) {
          CodeGen->EmitFunctionCall("pluss_parallel", {"reuse", "1"});
//          CodeGen->EmitFunctionCall("pluss_parallel_histogram_update", {"no_share_intra_thread_RI", "reuse", "1"});
        } else {
          ...
          case FULL_SHARE: {
            ...
            CodeGen->EmitCode("if (distance_to(reuse,0) > distance_to(reuse," + parallel_iteration_cnt + ")) {");
            CodeGen->tab_count++;
            CodeGen->EmitFunctionCall("pluss_parallel_histogram_update", {"share_intra_thread_RI[THREAD_NUM-1]", "reuse", "1"});
            CodeGen->tab_count--;
            CodeGen->EmitCode("} else {");
            CodeGen->tab_count++;
            CodeGen->EmitFunctionCall("pluss_parallel_histogram_update", {"no_share_intra_thread_RI", "reuse", "1"});
          ...
          case SPATIAL_SHARE: {
            ...
            CodeGen->EmitCode("if (distance_to(reuse,0) > distance_to(reuse," + parallel_iteration_cnt + ")) {");
            CodeGen->tab_count++;
            CodeGen->EmitFunctionCall("pluss_parallel_histogram_update", {"share_intra_thread_RI[CLS/(DS*CHUNK_SIZE)-1]", "reuse", "1"});
            CodeGen->tab_count--;
            CodeGen->EmitCode("} else {");
            CodeGen->tab_count++;
            CodeGen->EmitFunctionCall("pluss_parallel_histogram_update", {"no_share_intra_thread_RI", "reuse", "1"});
        ...
        CodeGen->EmitCode("}");
        if (InterleavingTechnique == 0) {
          CodeGen->EmitStmt("LAT_" + RefNode->getArrayNameString() +
                            "[addr] = count");
          CodeGen->EmitDebugInfo("LATIterMap[count] = access");
          CodeGen->EmitStmt("count += 1");
        } else {
          CodeGen->EmitStmt("LAT_" + RefNode->getArrayNameString() +
                            "[tid_to_run][addr] = count[tid_to_run]");
          CodeGen->EmitDebugInfo("LATIterMap[tid_to_run][count[tid_to_run]] = access");
...
//      } // check if there is a branch opened
    ...
    if (G->QueryEngine->isLastAccessInLoop(currentNode) ||
      ...
      codess.str("");
      if (BranchTNode * Branch = dynamic_cast<BranchTNode *>(currentNode)) {
        ParallelSamplerBranchBodyGenImpl(dynamic_cast<DummyTNode *>(Branch->neighbors[0]),
                                         type, nextNode);
        // need to generate the access inside the branch
        if (!Branch->neighbors[1]->neighbors.empty()) {
          ParallelSamplerBranchBodyGenImpl(dynamic_cast<DummyTNode *>(Branch->neighbors[1]),
                                           type, nextNode);
        }
      }
    }
  ...
  CodeGen->EmitCode("if (idle_threads.size() == THREAD_NUM && !dispatcher.hasNextChunk("
                    +to_string(scheduling==STATIC)+")) {");
  CodeGen->tab_count++;
  CodeGen->EmitDebugInfo("cout << \"No chunk available, sampler terminates\" << endl");
  CodeGen->EmitStmt("break");
  CodeGen->tab_count--;
  ...
  CodeGen->EmitStmt("idle_threads.clear()");
  CodeGen->EmitStmt("worker_threads.clear()");
  // we clear all LAT entries to remove all inter parallel reuses.
  vector<RefTNode *> References;
  unordered_set<string> ArrayNameInLoop;
...
void ModelCodeGenWrapperPass::PerRefSamplerBodyGenImpl(SPSTNode *Root,
                                                         RefTNode *TargetRef)
{
  if (LoopTNode *LoopNode = dynamic_cast<LoopTNode *>(Root)) {
    ...
    if (QueryEngine->isReachable(LoopNode, TargetRef)) {
...
//                                    {"this_thread::get_id()", loop_acc_count_expr});
//          CodeGen->EmitStmt("m.unlock()");
//        } else {
//          CodeGen->EmitFunctionCall("pluss_bypass", {loop_acc_count_expr});
//        }
        CodeGen->tab_count--;
...
//          CodeGen->EmitStmt("m.lock()");
//          CodeGen->EmitFunctionCall("pluss_per_thread_bypass",
//                                    {"this_thread::get_id()", false_branch_acc_count_expr});
//          CodeGen->EmitStmt("m.unlock()");
//        } else {
//          CodeGen->EmitFunctionCall("pluss_bypass", {false_branch_acc_count_expr});
//        }
        CodeGen->tab_count--;
        CodeGen->EmitCode("} // end of start_reuse_search");
      }
    ...
    if (QueryEngine->areAccessToSameArray(RefNode, TargetRef)) {
      ...
      if (EnableModelOpt && InterleavingTechnique == 0) {
        LoopTNode *SrcParallelLoop = QueryEngine->GetParallelLoopDominator(TargetRef);
        if (!SrcParallelLoop) {
          // if reuse src is in sequential region and reuse sink is in parallel
          // region, when find a reuse, the parallel reuse is splited in
          // two parts.
          // the sequential part is sequential_count - sequentialLAT[src]
          // the parallel part is f(parallel_count) where parallel_count is
          // the number of accesses between the first access in parallel region
          // and the reuse sink
          if (LoopTNode *ParallelParent =
              QueryEngine->GetParallelLoopDominator(RefNode)) {
            if (ModelValidateLoops.find(ParallelParent) !=
                ModelValidateLoops.end()) {
              SmallVector<SPSTNode *, 4> FirstParallelAccesses,
                  LastParallelAccesses;
              FirstParallelAccesses =
                  QueryEngine->GetFirstAccessInLoop(ParallelParent);
              LastParallelAccesses =
                  QueryEngine->GetLastAccessInLoop(ParallelParent);
              AccPath path;
              QueryEngine->GetPath(RefNode, ParallelParent, path, false);
              string loop_induction = "", loop_init_expr = "0",
                  loop_final_expr = "N", loop_step = "1";
              string tmp = "";
              TranslateStatus status = SUCCESS;
              if (find(FirstParallelAccesses.begin(),
                       FirstParallelAccesses.end(),
                       RefNode) != FirstParallelAccesses.end()) {
                auto pathIter = path.rbegin();
                while (pathIter != path.rend()) {
                  LoopTNode *NestLoop = dynamic_cast<LoopTNode *>(*pathIter);
                  assert(
                      ...
                      "All node in a path should be an instance of LoopTNode");
                  tmp = Translator->ValueToStringExpr(
                      NestLoop->getInductionPhi(), status);
                  if (status == SUCCESS)
                    loop_induction = tmp;
                  tmp = Translator->ValueToStringExpr(
                      NestLoop->getLoopBound()->InitValue, status);
                  if (status == SUCCESS)
                    loop_init_expr = tmp;
                  ss << loop_induction << "==" << loop_init_expr;
                  if (pathIter + 1 != path.rend())
                    ss << " && ";
                  pathIter++;
                }
                CodeGen->EmitCode("if (" + ss.str() + ") {");
                ss.str("");
                CodeGen->tab_count++;
                CodeGen->EmitStmt("parallel_count = 0");
                CodeGen->tab_count--;
                CodeGen->EmitCode("}");
              } else if (find(LastParallelAccesses.begin(),
                              LastParallelAccesses.end(),
                              RefNode) != LastParallelAccesses.end()) {
                auto pathIter = path.rbegin();
                while (pathIter != path.rend()) {
                  LoopTNode *NestLoop = dynamic_cast<LoopTNode *>(*pathIter);
                  assert(
                  ...
                  if (status == SUCCESS)
                    loop_init_expr = tmp;
                  tmp = Translator->ValueToStringExpr(
                      NestLoop->getLoopBound()->FinalValue, status);
                  if (status == SUCCESS)
                    loop_final_expr = tmp;
                  tmp = Translator->ValueToStringExpr(
                      NestLoop->getLoopBound()->StepValue, status);
                  if (status == SUCCESS)
                    loop_step = tmp;
                  ss << loop_induction << "==";
                  ss << "((" << loop_final_expr << "-" << loop_init_expr << ")/"
                     << loop_step << ")*" << loop_step << "+" << loop_init_expr;
                  switch (NestLoop->getLoopBound()->Predicate) {
                  case llvm::CmpInst::ICMP_UGT:
                  case llvm::CmpInst::ICMP_SGT:
                  case llvm::CmpInst::ICMP_ULT:
                  case llvm::CmpInst::ICMP_SLT:
                ...
                ss.str("");
                CodeGen->tab_count++;
                CodeGen->EmitStmt("sequential_count += parallel_count");
                CodeGen->tab_count--;
                CodeGen->EmitCode("}");
              }
            }
...
//          ss << "subscripts[" << i << "]";
        ...
        if (EnableModelOpt && InterleavingTechnique == 0) {
          // ModelGen is enabled if
          // 1) EnableModelOpt option is given and
          // 2) The loop is under OpenMP static and uniform interleaving
          LoopTNode *SrcParallelLoop = QueryEngine->GetParallelLoopDominator(TargetRef);
          LoopTNode *SinkParallelLoop = QueryEngine->GetParallelLoopDominator(RefNode);
          // now we model the reuse f()
          // if both reuse src and sink are in parallel region, we apply the model
          // directly
          if (SrcParallelLoop && SinkParallelLoop) {
            parallel_model_is_generated = true;
            if (ModelValidateLoops.find(SrcParallelLoop) != ModelValidateLoops.end()
                && ModelValidateLoops.find(SinkParallelLoop) != ModelValidateLoops.end()) {
              CodeGen->EmitStmt("parallel_src_parallel_sink_cnt++");
            }
            ModelGen(TargetRef, SrcParallelLoop, RefNode, SinkParallelLoop, false);
            CodeGen->EmitLabel("PARALLEL_REUSE_UPDATE_" +
                               SPSNodeNameTable[RefNode]);
          } else if (SinkParallelLoop) {
            // if reuse src is in sequential region and sink is in parallel region,
            // need to apply the model to parallel_count at the reuse sink
            // parallel_reuse = f(parallel_count) + sequential_count
            parallel_model_is_generated = true;
            if (ModelValidateLoops.find(SinkParallelLoop) != ModelValidateLoops.end()) {
              CodeGen->EmitStmt("sequential_src_parallel_sink_cnt ++");
              SmallVector<SPSTNode *, 4> FirstAccessesInParallelLoop;
              FirstAccessesInParallelLoop = QueryEngine->GetFirstAccessInLoop(SinkParallelLoop);
              if (RefTNode *FakeSrc = dynamic_cast<RefTNode *>(FirstAccessesInParallelLoop.front())) {
                // Generate code to check if current iteration is the first access
                // of the SrcParallelLoop
//                CodeGen->EmitComment("Generate model between " +
//                                     SPSNodeNameTable[FakeSrc] + " and " +
//                                     SPSNodeNameTable[RefNode]);
//                CodeGen->EmitComment(
//                    "compute the parallel reuse between the first access "
//                    "in the parallel region and the sink");
//                ModelGen(FakeSrc, SinkParallelLoop, RefNode, SinkParallelLoop,
//                         true);
//                CodeGen->EmitStmt(
//                    "parallel_reuse += (sequential_count - sequentialLAT[src])");
                CodeGen->EmitStmt("int parallel_reuse = -1");
              }
              CodeGen->EmitLabel("PARALLEL_REUSE_UPDATE_" +
                                 SPSNodeNameTable[RefNode]);
            }
          } else if (SrcParallelLoop) {
            // if reuse src is in parallel region and sink is in sequential region,
            // need to apply the model to parallel_count at the reuse src and the
            // last access of the parallel region
            // parallel_reuse = f(parallel_count) + sequential_count
            if (ModelValidateLoops.find(SrcParallelLoop) != ModelValidateLoops.end()) {
              CodeGen->EmitStmt("parallel_src_sequential_sink_cnt ++");
//              ModelGen(TargetRef, SrcParallelLoop, FakeSink, SrcParallelLoop, true);
//              CodeGen->EmitStmt("parallel_reuse += sequential_count");
              CodeGen->EmitStmt("int parallel_reuse = -1");
            }
            CodeGen->EmitLabel("PARALLEL_REUSE_UPDATE_" +
                               SPSNodeNameTable[RefNode]);
          } else {
            CodeGen->EmitStmt("sequential_src_sequential_sink_cnt ++");
            CodeGen->EmitComment(SPSNodeNameTable[TargetRef]+" and "+SPSNodeNameTable[RefNode]+" are both in sequential region, no need to apply the model");
          }
        }
        if (parallel_model_is_generated) {
          CodeGen->EmitDebugInfo("cout << \"[\" << reuse_type << \"] \" << reuse << \" -> \" << parallel_reuse << \" left \" << samples.size() << endl");
          CodeGen->EmitCode("if (parallel_reuse > 0) {");
          CodeGen->tab_count++;
          if (EnableParallelOpt)
            CodeGen->EmitStmt("pluss_parallel_histogram_update(histogram, parallel_reuse, 1)");
          else
            CodeGen->EmitStmt("pluss_histogram_update(parallel_reuse, 1.)");
          CodeGen->tab_count--;
          CodeGen->EmitCode("}");
...
//        CodeGen->EmitStmt("m.unlock()");
      ...
      if (EnableModelOpt && InterleavingTechnique == 0) {
        LoopTNode *SrcParallelLoop = QueryEngine->GetParallelLoopDominator(TargetRef);
        LoopTNode *SinkParallelLoop = QueryEngine->GetParallelLoopDominator(RefNode);
        // need to collect parallel_count if and only if the reuse src is in
        ...
        if (SrcParallelLoop && !SinkParallelLoop) {
          SmallVector<SPSTNode *, 4> LastAccessesInParallelLoop;
          LastAccessesInParallelLoop = QueryEngine->GetLastAccessInLoop(SrcParallelLoop);
          if (find(LastAccessesInParallelLoop.begin(), LastAccessesInParallelLoop.end(), RefNode) != LastAccessesInParallelLoop.end()) {
            CodeGen->EmitComment("Generate model between "+SPSNodeNameTable[TargetRef]+" and "+SPSNodeNameTable[RefNode]);
#if 0
            // Generate code to check if current iteration is the last access
            // of the SrcParallelLoop
            CodeGen->EmitCode("if (/* this is the last iteration */) {");
            CodeGen->tab_count++;
            CodeGen->EmitComment("compute the parallel reuse between the src and the last "
                                 "access in the parallel region");
            ModelGen(TargetRef, SrcParallelLoop, RefNode, SrcParallelLoop, true);
            CodeGen->tab_count--;
            CodeGen->EmitCode("}");
#endif
            QueryEngine->GetPath(RefNode, SrcParallelLoop, path, false);
            auto pathIter = path.rbegin();
            while (pathIter != path.rend()) {
              LoopTNode *NestLoop = dynamic_cast<LoopTNode *>(*pathIter);
              assert(
            ...
            ss.str("");
            CodeGen->tab_count++;
            // generate the model to compute the fake parallel reuse between reuse
            // src and the last access of the parallel region.
            ModelGen(TargetRef, SrcParallelLoop, RefNode, SrcParallelLoop, true);
            CodeGen->tab_count--;
            CodeGen->EmitCode("}");
          }
...
//        CodeGen->EmitStmt("m.lock()");
//        CodeGen->EmitFunctionCall("pluss_per_thread_bypass",
//                                  {"this_thread::get_id()" ,"1"});
//        CodeGen->EmitStmt("m.unlock()");
//      } else {
//        CodeGen->EmitFunctionCall("pluss_bypass", {"1"});
//      }
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
    }
...
void ModelCodeGenWrapperPass::PerRefParallelSamplerBodyGenImpl(SPSTNode *Root,
...
void ModelCodeGenWrapperPass::ParallelSamplerSamplingHeaderGen(LoopTNode *ParallelLoop,
                                                                 AccGraph *G,
                                                                 RefTNode *TargetRef,
                                                                 Interleaving type,
...
void ModelCodeGenWrapperPass::ParallelSamplerBranchBodyGenImpl(DummyTNode *Branch,
                                                                 Interleaving type,
                                                                 SPSTNode *MergePoint)
{
  ...
  while (branchIter != Branch->neighbors.end()) {
    if (RefTNode *Ref = dynamic_cast<RefTNode *>(*branchIter)) {
      codess << "if (progress[tid_to_run]->ref == \"" << SPSNodeNameTable[Ref] << "\") {";
      CodeGen->EmitCode(codess.str());
      codess.str("");
      CodeGen->tab_count++;
      vector<string> params = EmitRefNodeAccessExpr(Ref, true);
      if (!params.empty()) {
        if (EnableSampling && SamplingMethod == SEQUENTIAL_START) {
          CodeGen->EmitComment("choose a number between 1 and 100");
          CodeGen->EmitStmt("int enable = rand() % 100 + 1");
        ...
        codess << "addr = GetAddress_" << SPSNodeNameTable[Ref] << "(";
        for (unsigned i = 0; i < Ref->getSubscripts().size(); i++) {
          codess << EmitRefNodeAccessExprAtIdx(Ref->getSubscripts()[i], true);
          if (i < Ref->getSubscripts().size()-1)
            codess << ",";
        }
        codess << ")";
        CodeGen->EmitStmt(codess.str());
        ...
        CodeGen->tab_count++;
        CodeGen->EmitStmt("cout << \"[\" << reuse << \"] " + SPSNodeNameTable[Ref] +"\" << endl");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->EmitHashMacro("#endif");
        CodeGen->EmitFunctionCall("pluss_histogram_update", {"reuse", "1."});
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->EmitStmt("LAT_"+Ref->getArrayNameString()+"[addr] = count");
        CodeGen->EmitStmt("count += 1");
      }
...
void ModelCodeGenWrapperPass::PerRefParallelSamplerBranchBodyGenImpl(DummyTNode *Branch,
                                                                       Interleaving type,
                                                                       SPSTNode *MergePoint,
                                                                       RefTNode *TargetRef)
{
...
ModelCodeGenWrapperPass::CheckDependenciesInPath(LoopTNode *Child, AccPath &Path)
{
...
bool ModelCodeGenWrapperPass::hasInductionVarDependenceChildren(LoopTNode *LoopNode)
{
  IVDepNode *ParentNode = nullptr;
  for (auto ivdep : getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>()
      .InductionVarDepForest) {
...
bool ModelCodeGenWrapperPass::hasParallelLoopInductionVar(Value *V)
{
  bool retVal = false;
  if (!V) {
    return false;
...
bool ModelCodeGenWrapperPass::isInParallelLoop(RefTNode *RefNode)
{
  AccPath path;
  QueryEngine->GetPath(RefNode, nullptr, path);
...
bool ModelCodeGenWrapperPass::runOnFunction(Function &F)
{
  ...
  PerLoopArraysMap =
      getAnalysis<ReferenceAnalysis::ReferenceAnalysis>().PerLoopArraysMap;
  ...
  HeaderGen();
  RefAddressFuncGen();
  ModelUtilGen(InterleavingTechnique == 0 && !ModelValidateLoops.empty());
  SamplerBodyGen(EnablePerReference || (EnableSampling && (SamplingMethod == RANDOM_START)));
  MainFuncGen();
  return false;
}

void ModelCodeGenWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
