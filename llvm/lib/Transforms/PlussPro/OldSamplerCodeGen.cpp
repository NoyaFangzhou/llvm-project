//
// Created by noya-fangzhou on 1/8/22.
// CRI Model w/ Old SPS sampling sampler codegen
//
...
#define DEBUG_TYPE "old-model-codegen"
...
namespace OldSamplerCodeGen {

char OldSamplerCodeGenWrapperPass::ID = 0;
static RegisterPass<OldSamplerCodeGenWrapperPass>
    X("old-model-codegen", "Pass that generate the model code w/ Island-hopping optimization");

OldSamplerCodeGenWrapperPass::OldSamplerCodeGenWrapperPass() : FunctionPass(ID) {}

// the trip is represented as the (ub - lb + 1) / step if <=/>= or
// (ub - lb) / step if </>
// default, the trip is (ub - lb), assuming < and step is 1
string OldSamplerCodeGenWrapperPass::RepresentLoopTrip(LoopTNode *LN)
{
  string trip = "";
...
int OldSamplerCodeGenWrapperPass::ComputeDistanceToParallelLoop(LoopTNode *LN)
{
...
int OldSamplerCodeGenWrapperPass::isInsideParallelLoopNest(PHINode *Phi)
{
...
ReuseType OldSamplerCodeGenWrapperPass::isInterThreadSharingRef(RefTNode *RefNode)
{
  // [i][i], [i][j], [j][k]
...
string OldSamplerCodeGenWrapperPass::ReplaceInductionVarExprByType(
...
string OldSamplerCodeGenWrapperPass::EmitRefNodeAccessExprAtIdx(Value *Index,
                                                              bool isInParallel) {
  TranslateStatus status = SUCCESS;
...
vector<string> OldSamplerCodeGenWrapperPass::EmitRefNodeAccessExpr(RefTNode *RefNode,
                                                                 bool isInParallel)
{
...
string OldSamplerCodeGenWrapperPass::EmitBranchCondExpr(BranchTNode *Branch)
{
  TranslateStatus status = SUCCESS;
  string cond = "true";
  stringstream  ss;
...
string OldSamplerCodeGenWrapperPass::EmitLoopNodeExpr(LoopTNode *LoopNode,
                                                    bool isSampledLoop)
{
  TranslateStatus status = SUCCESS;
...
string OldSamplerCodeGenWrapperPass::EmitOneParallelIterationTripFormula(LoopTNode *LoopNode)
{
  if (!LoopNode)
    return "0";
  stringstream ss;
...
string OldSamplerCodeGenWrapperPass::EmitTripFormula(SPSTNode *Node)
{
  string trip_formula = "";
...
string OldSamplerCodeGenWrapperPass::EmitLoopTripExpr(LoopTNode *L)
{
...
bool OldSamplerCodeGenWrapperPass::canBeSimplifiedByFormula(SPSTNode *Node)
{
...
void OldSamplerCodeGenWrapperPass::ModelUtilGen()
{
  ...
  */
  ...
  if (EnableParallelOpt) {
    CodeGen->EmitCode(
        "void no_share_distribute(unordered_map<uint64_t, double>& histogram_to_distribute, unordered_map<uint64_t, double>& target_histogram, int thread_cnt=THREAD_NUM)");
  } else {
    CodeGen->EmitCode(
        "void no_share_distribute(unordered_map<uint64_t, double>& histogram_to_distribute, int thread_cnt=THREAD_NUM)");
  }
  CodeGen->EmitCode("{");
  CodeGen->tab_count++;
  ...
  if (EnableParallelOpt) {
    CodeGen->EmitCode("void share_distribute(unordered_map<int, unordered_map<uint64_t, double>>&histogram_to_distribute, unordered_map<uint64_t, double> &target_histogram, int thread_cnt=THREAD_NUM)");
  } else {
    CodeGen->EmitCode("void share_distribute(unordered_map<int, unordered_map<uint64_t, double>>&histogram_to_distribute, int thread_cnt=THREAD_NUM)");
  }
  CodeGen->EmitCode("{");
  CodeGen->tab_count++;
...
void OldSamplerCodeGenWrapperPass::HeaderGen()
{
  ...
  if ((EnableSampling && SamplingMethod == RANDOM_START) || EnablePerReference) {
    ...
    if (!EnableParallelOpt) {
      ...
      CodeGen->EmitStmt(
          "unordered_map<int, unordered_map<uint64_t, double>> share_intra_thread_RI");
    }
  } else {
...
void OldSamplerCodeGenWrapperPass::RefAddressFuncGen()
{
...
void OldSamplerCodeGenWrapperPass::MainFuncGen()
{
...
void OldSamplerCodeGenWrapperPass::SamplerBodyGen(bool isPerReference)
{
...
//  CodeGen->EmitProgressClassDef();
  ...
  for (auto ref : SPSNodeNameTable) {
    if (RefTNode *TargetRef = dynamic_cast<RefTNode *>(ref.first)) {
      ...
      CodeGen->EmitStmt("array<uint64_t, THREAD_NUM> count = {0}");
      ...
      if (EnableParallelOpt) {
        CodeGen->EmitStmt(
            "unordered_map<uint64_t, double> no_share_histogram");
        CodeGen->EmitStmt("unordered_map<int, unordered_map<uint64_t, double>> share_histogram");
      }
...
//            LLVM_DEBUG(dbgs() << "check loop" << Loop->getLoopStringExpr() << "\n");
          if (Loop != *(path.rbegin())) {
            ...
            if (Loop == *(path.rbegin())) {
              ...
              CodeGen->EmitStmt("vector<Iteration> samples");
              ...
              CodeGen->EmitComment("generate all samples and put "
                                   "them in a list");
...
//                                    induction_expr + ") + \"-\")");
              ...
              CodeGen->EmitStmt(ss.str());
              ss.str(""); // clear the stream to generate other sampling stmts
              CodeGen->EmitStmt("samples.push_back(sample)");
#if 0
...
#endif
              CodeGen->EmitComment("start from the sample generated");
              CodeGen->EmitStmt("start_reuse_search = false");
              CodeGen->EmitStmt("start_iteration = sample");
...
void OldSamplerCodeGenWrapperPass::SamplerBodyGenImpl(SPSTNode *Root)
{
...
void OldSamplerCodeGenWrapperPass::LoopIterUpdateGen(AccGraph *G,
                                                   AccGraphEdge *E,
                                                   LoopTNode* IDomLoop,
...
void OldSamplerCodeGenWrapperPass::PerRefSamplerBodyGenImpl(SPSTNode *Root,
                                                          RefTNode *TargetRef)
{
  if (LoopTNode *LoopNode = dynamic_cast<LoopTNode *>(Root)) {
    ...
    if (QueryEngine->isReachable(LoopNode, TargetRef) && find(dominators.begin(), dominators.end(), LoopNode) != dominators.end()) {
      ...
      if (!canBeSimplifiedByFormula(LoopNode)) {
        ...
        if (InterleavingTechnique == 0)
          ...
          CodeGen->EmitStmt("count[tid_to_run] += (uint64_t)(" + loop_acc_count_expr + "/ THREAD_NUM)");
        CodeGen->tab_count--;
...
//          ss << "subscripts[" << i << "]";
        ...
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          ...
          if (RefNode == TargetRef) {
            ...
            CodeGen->tab_count--;
            CodeGen->EmitCode("} /* end of access->compare(start_iteration) == 0 */");
          }
        }
        ...
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          ...
          CodeGen->EmitComment("stop traversing if reuse of all samples"
                               "are found");
          CodeGen->EmitDebugInfo("if (s >= num_samples) { cout << \"[\" << reuse << \"] for last sample \" << src->toString() << endl; }");
          CodeGen->EmitCode("if (s >= num_samples) { goto END_SAMPLE; }");
        }
        CodeGen->tab_count--;
...
void OldSamplerCodeGenWrapperPass::PerRefParallelSamplerBodyGenImpl(SPSTNode *Root,
...
//            CodeGen->EmitStmt("pluss_histogram_update(reuse, 1)");
          ...
          if (EnableSampling && SamplingMethod == RANDOM_START) {
            ...
            CodeGen->EmitComment("move to the next sample");
            CodeGen->EmitStmt("s++");
            CodeGen->EmitComment("stop traversing if reuse of all samples"
                                 "are found");
            CodeGen->EmitDebugInfo(
                "if (s >= num_samples) { cout << \"[\" << reuse << \"] for last sample \" << src->toString() << endl; }");
            CodeGen->EmitCode("if (s > num_samples) { goto END_SAMPLE; }");
            CodeGen->EmitDebugInfo(
                "cout << \"delete sample \" << src->toString() << \", remain:\" << num_samples - s << endl");
            ...
            CodeGen->EmitHashMacro("#if defined(DEBUG)");
            CodeGen->EmitStmt("cout << \"Jump to next sample \" endl");
            CodeGen->EmitHashMacro("#endif");
...
void OldSamplerCodeGenWrapperPass::ParallelSamplerSamplingHeaderGen(LoopTNode *ParallelLoop,
                                                                  AccGraph *G,
                                                                  RefTNode *TargetRef,
                                                                  Interleaving type,
...
OldSamplerCodeGenWrapperPass::CheckDependenciesInPath(LoopTNode *Child, AccPath &Path)
{
...
bool OldSamplerCodeGenWrapperPass::hasInductionVarDependenceChildren(LoopTNode *LoopNode)
{
  IVDepNode *ParentNode = nullptr;
  for (auto ivdep : getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>()
      .InductionVarDepForest) {
...
bool OldSamplerCodeGenWrapperPass::hasParallelLoopInductionVar(Value *V)
{
  bool retVal = false;
  if (!V) {
    return false;
...
bool OldSamplerCodeGenWrapperPass::isInParallelLoop(RefTNode *RefNode)
{
  AccPath path;
  QueryEngine->GetPath(RefNode, nullptr, path);
...
bool OldSamplerCodeGenWrapperPass::runOnFunction(Function &F)
{
...
void OldSamplerCodeGenWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
