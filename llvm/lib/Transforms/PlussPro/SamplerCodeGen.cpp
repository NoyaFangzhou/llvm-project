...
#define DEBUG_TYPE "sampler-codegen"

//cl::opt<int> SamplingRatio(
//    "sample-rate", cl::init(100), cl::NotHidden,
//    cl::desc("The sampling ratio in percentage unit, i.e. 10 means 10%"));
//
cl::opt<int> SamplingInterval(
    "sample-interval", cl::init(100), cl::NotHidden,
    cl::desc("Number of accesses to sample. (only bursty sampling). 100"
             "means tracking reuses after every 100 memory accesses"));
...
//    cl::desc("Enable the RI model optimization"));

STATISTIC(ReplaceByProgressObj, "Number of loop lower bounds"
                                        " replaced by progress obj iteration "
                                        "tuple");
STATISTIC(ReplaceLoopLBByProgressObj, "Number of loop lower bounds"
                                        " replaced by progress obj iteration "
                                        "tuple");
STATISTIC(ReplaceLoopUBByProgressObj, "Number of loop upper bounds"
                                        " replaced by progress obj iteration "
                                        "tuple");
STATISTIC(ReplaceLoopIncByProgressObj, "Number of loop step instructions"
                                        " replaced by progress obj iteration "
                                        "tuple");
STATISTIC(NumSamplerSkipped, "Number of sampler functions that are skipped "
                             "when the per reference code generation is enabled");

STATISTIC(NumStaticParallelLoop, "Number of parallel loop nests whose reuse can be"
                                 "derived by model");

using namespace std;
using namespace llvm;

namespace SamplerCodeGen {

char SamplerCodeGenWrapperPass::ID = 0;
static RegisterPass<SamplerCodeGenWrapperPass>
    X("sampler-codegen", "Pass that generate the sampler code");

SamplerCodeGenWrapperPass::SamplerCodeGenWrapperPass() : FunctionPass(ID) {}

// the trip is represented as the (ub - lb + 1) / step if <=/>= or
// (ub - lb) / step if </>
// default, the trip is (ub - lb), assuming < and step is 1
string SamplerCodeGenWrapperPass::RepresentLoopTrip(LoopTNode *LN)
{
  string trip = "";
...
int SamplerCodeGenWrapperPass::ComputeDistanceToParallelLoop(LoopTNode *LN)
{
...
int SamplerCodeGenWrapperPass::isInsideParallelLoopNest(PHINode *Phi)
{
...
string SamplerCodeGenWrapperPass::ReplaceInductionVarExprByType(
...
string SamplerCodeGenWrapperPass::EmitRefNodeAccessExprAtIdx(Value *Index,
                                                                bool isInParallel) {
  TranslateStatus status = SUCCESS;
...
vector<string> SamplerCodeGenWrapperPass::EmitRefNodeAccessExpr(RefTNode *RefNode,
                                                        bool isInParallel)
{
...
string SamplerCodeGenWrapperPass::EmitBranchCondExpr(BranchTNode *Branch)
{
  TranslateStatus status = SUCCESS;
  string cond = "true";
  stringstream  ss;
...
string SamplerCodeGenWrapperPass::EmitLoopNodeExpr(LoopTNode *LoopNode,
bool isSampledLoop)
{
  TranslateStatus status = SUCCESS;
...
string SamplerCodeGenWrapperPass::EmitOneParallelIterationTripFormula(LoopTNode *LoopNode)
{
  if (!LoopNode)
    return "0";
  stringstream ss;
...
string SamplerCodeGenWrapperPass::EmitTripFormula(SPSTNode *Node)
{
  string trip_formula = "";
...
string SamplerCodeGenWrapperPass::EmitLoopTripExpr(LoopTNode *L)
{
...
bool SamplerCodeGenWrapperPass::canBeSimplifiedByFormula(SPSTNode *Node)
{
...
void SamplerCodeGenWrapperPass::ModelUtilGen()
{
  CodeGen->EmitCode("int getThreadID(int i, int start, int step)");
  CodeGen->EmitCode("{");
  CodeGen->tab_count++;
  CodeGen->EmitStmt("return ((i-start)/step) / CHUNK_SIZE - (floor(((i-start)/step) / (CHUNK_SIZE * THREAD_NUM)) * THREAD_NUM)");
  CodeGen->tab_count--;
  CodeGen->EmitCode("}");
  CodeGen->EmitCode("int getChunkID(int i, int start, int step)");
  CodeGen->EmitCode("{");
  CodeGen->tab_count++;
  CodeGen->EmitStmt("return floor(((i-start)/step) / (CHUNK_SIZE * THREAD_NUM))");
  CodeGen->tab_count--;
  CodeGen->EmitCode("}");
  ...
  CodeGen->EmitCode("{");
  CodeGen->tab_count++;
  CodeGen->EmitStmt("return (trip % (CHUNK_SIZE*THREAD_NUM)) == 0 ? trip / (CHUNK_SIZE*THREAD_NUM) : trip / (CHUNK_SIZE*THREAD_NUM) + 1");
  CodeGen->tab_count--;
  CodeGen->EmitCode("}");
...
void SamplerCodeGenWrapperPass::ModelGen(RefTNode *Src, LoopTNode *SrcParallelLoop,
...
string SamplerCodeGenWrapperPass::MiddleAccessCount(LoopTNode *SrcLoop, LoopTNode *SinkLoop)
{
  if (SrcLoop == SinkLoop)
    return "0";
...
void SamplerCodeGenWrapperPass::HeaderGen()
{
...
void SamplerCodeGenWrapperPass::RefAddressFuncGen()
{
...
void SamplerCodeGenWrapperPass::MainFuncGen()
{
...
//          CodeGen->EmitFunctionCall("pluss_timer_print", {});
  ...
  if (EnableParallelOpt)
    CodeGen->EmitFunctionCall("pluss_parallel_AET", {"histogram"});
  else
    CodeGen->EmitFunctionCall("pluss_AET", {});
  CodeGen->EmitFunctionCall("pluss_timer_stop", {});
  CodeGen->EmitFunctionCall("pluss_timer_print", {});
  if (EnableParallelOpt)
    CodeGen->EmitFunctionCall("pluss_parallel_print_histogram", {"histogram"});
  else
...
void SamplerCodeGenWrapperPass::SamplerBodyGen(bool isPerReference)
{
...
//  CodeGen->EmitProgressClassDef();
  if (EnableSampling) {
    ...
    case SEQUENTIAL_START:
      ...
      break;
    case BURSTY:
      sampler_typess << "Use bursty sampling every "
                     << SamplingInterval << " accesses";
      break;
    default:
  ...
  if (!isPerReference) {
    ...
    CodeGen->EmitStmt("int tid_to_run = 0, start_tid=0");
    CodeGen->EmitStmt("uint64_t count = 0, addr = 0");
#if 0
    ...
    CodeGen->EmitCode("for (auto entry : LATIterMap) { delete entry.second; }");
...
//            CodeGen->EmitFunctionCall("pluss_reset", {});
          CodeGen->EmitStmt("iteration_traversed_map[\""+ref.second+"\"] = count");
...
void SamplerCodeGenWrapperPass::SamplerBodyGenImpl(SPSTNode *Root)
{
  if (LoopTNode *LoopNode = dynamic_cast<LoopTNode *>(Root)) {
    if (LoopNode->isParallelLoop()) {
      ...
      if (InterleavingTechnique == 0)
        ...
        ParallelSamplerBodyGenImpl(LoopNode, Graphs[LoopNode], RANDOM_INTERLEAVING);
    } else {
    ...
    if (!params.empty()) {
      if (EnableSampling && SamplingMethod == SEQUENTIAL_START) {
        ...
        CodeGen->tab_count++;
      } else if (EnableSampling && SamplingMethod == BURSTY) {
...
//        ss << "subscripts[" << i << "]";
      ...
      ss.str("");
      CodeGen->EmitCode("if (LAT_"+RefNode->getArrayNameString()+".find(addr) != LAT_"+RefNode->getArrayNameString()+".end()) {");
      CodeGen->tab_count++;
      CodeGen->EmitStmt("uint64_t reuse = count - LAT_"+RefNode->getArrayNameString()+"[addr]");
      CodeGen->EmitHashMacro("#if defined(DEBUG)");
      ...
      CodeGen->EmitHashMacro("#endif");
      CodeGen->EmitFunctionCall("pluss_histogram_update", {"reuse", "1.0"});
      CodeGen->tab_count--;
      CodeGen->EmitCode("}");
...
void SamplerCodeGenWrapperPass::LoopIterUpdateGen(AccGraph *G,
                                             AccGraphEdge *E,
                                             LoopTNode* IDomLoop,
...
void SamplerCodeGenWrapperPass::ParallelSamplerBodyGenImpl(SPSTNode *Root,
                                                           AccGraph *G,
                                                           Interleaving type)
{
...
//      } // check if there is a branch opened
  ...
  for (auto array : PerLoopArraysMap[ParallelLoopNode]) {
    CodeGen->EmitStmt("LAT_" + array + ".clear()");
  }
  CodeGen->EmitCode("for (unsigned i = 0; i < progress.size(); i++) {");
  CodeGen->tab_count++;
  CodeGen->EmitCode("if (progress[i]) {");
...
void SamplerCodeGenWrapperPass::PerRefSamplerBodyGenImpl(SPSTNode *Root,
RefTNode *TargetRef)
{
  if (LoopTNode *LoopNode = dynamic_cast<LoopTNode *>(Root)) {
    ...
    if (QueryEngine->isReachable(LoopNode, TargetRef)) {
      ...
      if (LoopNode->isParallelLoop()) {
        ...
        if (InterleavingTechnique == 0)
          ...
          PerRefParallelSamplerBodyGenImpl(
              LoopNode, TargetRef, Graphs[LoopNode], RANDOM_INTERLEAVING);
        //        PerRefParallelSamplerBodyGenImpl(LoopNode, TargetRef, Graphs[LoopNode]);
...
void SamplerCodeGenWrapperPass::PerRefParallelSamplerBodyGenImpl(SPSTNode *Root,
...
void SamplerCodeGenWrapperPass::ParallelSamplerSamplingHeaderGen(LoopTNode *ParallelLoop,
                                                                 AccGraph *G,
                                                                 RefTNode *TargetRef,
                                                                 Interleaving type,
...
void SamplerCodeGenWrapperPass::ParallelSamplerBranchBodyGenImpl(DummyTNode *Branch,
                                                                 Interleaving type,
                                                                 SPSTNode *MergePoint)
{
...
void SamplerCodeGenWrapperPass::PerRefParallelSamplerBranchBodyGenImpl(DummyTNode *Branch,
                                                                       Interleaving type,
                                                                       SPSTNode *MergePoint,
                                                                       RefTNode *TargetRef)
{
...
pair<LoopTNode *, LoopTNode *>
    SamplerCodeGenWrapperPass::CheckDependenciesInPath(LoopTNode *Child, AccPath &Path)
{
...
bool SamplerCodeGenWrapperPass::hasInductionVarDependenceChildren(LoopTNode *LoopNode)
{
  IVDepNode *ParentNode = nullptr;
  for (auto ivdep : getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>()
      .InductionVarDepForest) {
...
bool SamplerCodeGenWrapperPass::hasParallelLoopInductionVar(Value *V)
{
  bool retVal = false;
  if (!V) {
    return false;
...
bool SamplerCodeGenWrapperPass::isInParallelLoop(RefTNode *RefNode)
{
  AccPath path;
  QueryEngine->GetPath(RefNode, nullptr, path);
...
bool SamplerCodeGenWrapperPass::runOnFunction(Function &F)
{
  ...
  PerLoopArraysMap = getAnalysis<ReferenceAnalysis::ReferenceAnalysis>().PerLoopArraysMap;
...
void SamplerCodeGenWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
