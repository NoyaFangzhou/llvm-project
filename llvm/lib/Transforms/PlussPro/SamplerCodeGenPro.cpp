...
#define DEBUG_TYPE "new-sampler-codegen"
...
char SamplerCodeGenProWrapperPass::ID = 0;
static RegisterPass<SamplerCodeGenProWrapperPass>
    X("new-sampler-codegen", "Pass that generate the sampler code");

SamplerCodeGenProWrapperPass::SamplerCodeGenProWrapperPass() : FunctionPass(ID) {}

// the trip is represented as the (ub - lb + 1) / step if <=/>= or
// (ub - lb) / step if </>
// default, the trip is (ub - lb), assuming < and step is 1
string SamplerCodeGenProWrapperPass::RepresentLoopTrip(LoopTNode *LN)
{
  string trip = "";
  ...
  string loop_step = ReplaceInductionVarExprByType(
      LN->getLoopBound()->StepValue, DEFAULT, status);
  if (loop_step != "1")
    trip = "(" + trip + " / " + loop_step + ")";
  return trip;
}

int SamplerCodeGenProWrapperPass::ComputeDistanceToParallelLoop(LoopTNode *LN)
{
...
int SamplerCodeGenProWrapperPass::isInsideParallelLoopNest(PHINode *Phi)
{
...
string SamplerCodeGenProWrapperPass::ReplaceInductionVarExprByType(
  ...
  case Instruction::PHI: {
    ...
    if (InductionVarNameTable.find(Phi) != InductionVarNameTable.end()) {
      ...
      switch (type) {
      case SAMPLE: {
        indvar_name = "sample_"+indvar_name;
      }
        break;
      case OMP_PARALLEL:{
...
string SamplerCodeGenProWrapperPass::EmitRefNodeAccessExprAtIdx(Value *Index,
                                                                bool isInParallel)
{
  TranslateStatus status = SUCCESS;
...
vector<string> SamplerCodeGenProWrapperPass::EmitRefNodeAccessExpr(RefTNode *RefNode,
                                                                bool isInParallel)
{
  ...
  if (status == SUCCESS) {
    ...
    for (auto Index : RefNode->getSubscripts()) {
      // not all indices are inside the parallel loop, for those induction variable outside the parallel loop, we use its original value (ValueToStringExpr)
...
string SamplerCodeGenProWrapperPass::EmitBranchCondExpr(BranchTNode *Branch)
{
  TranslateStatus status = SUCCESS;
  string cond = "true";
  stringstream  ss;
...
string SamplerCodeGenProWrapperPass::EmitLoopNodeExpr(LoopTNode *LoopNode,
                                                   bool isSampledLoop)
{
  TranslateStatus status = SUCCESS;
...
string SamplerCodeGenProWrapperPass::EmitOneParallelIterationTripFormula(LoopTNode *LoopNode)
{
  if (!LoopNode)
    return "0";
  stringstream ss;
...
string SamplerCodeGenProWrapperPass::EmitTripFormula(SPSTNode *Node)
{
  string trip_formula = "";
...
string SamplerCodeGenProWrapperPass::EmitLoopTripExpr(LoopTNode *L)
{
...
bool SamplerCodeGenProWrapperPass::canBeSimplifiedByFormula(SPSTNode *Node)
{
...
void SamplerCodeGenProWrapperPass::HeaderGen()
{
  ...
  if ((EnableSampling && SamplingMethod == RANDOM_START)) {
    CodeGen->EmitStmt("random_device dev");
    CodeGen->EmitStmt(
...
void SamplerCodeGenProWrapperPass::RefAddressFuncGen()
{
...
void SamplerCodeGenProWrapperPass::MainFuncGen()
{
  ...
  if (EnableSampling && SamplingMethod == RANDOM_START) {
    if (EnableParallelOpt) {
      CodeGen->EmitStmt("unordered_map<uint64_t, uint64_t> histogram");
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          CodeGen->EmitStmt("unordered_map<uint64_t, uint64_t> histogram_" + node.second);
        }
      }
...
#if 0
      for (auto array: ArrayNames) {
        CodeGen->EmitStmt("unordered_map<uint64_t, uint64_t> LAT_"+array);
      }
#endif
//      CodeGen->EmitStmt("unordered_set<string> samples");
      CodeGen->EmitStmt("priority_queue<Iteration, vector<Iteration>, IterationComp> samples");
      CodeGen->EmitStmt("unordered_set<string> sample_names");
      CodeGen->EmitStmt("Iteration start_iteration");
#if 0
      CodeGen->EmitComment("Container of samples for each reference");
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          if (!QueryEngine->GetImmdiateLoopDominator(ref)) { continue; }
          CodeGen->EmitStmt("unordered_set<string> samples_"+node.second);
        }
      }
#endif
      CodeGen->EmitFunctionCall("pluss_timer_start", {});
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          ...
          // top level reference does not have dominator
          if (!QueryEngine->GetImmdiateLoopDominator(ref)) {
            CodeGen->EmitComment("Skip generating samples for " + ref->getRefExprString());
            continue;
          }
          if (ref == FirstAccess) {
            CodeGen->EmitFunctionCall("samplegen_" + node.second,
                                      {"ref(samples)", "ref(sample_names)", "ref(start_iteration)"});
          } else {
            CodeGen->EmitFunctionCall("samplegen_" + node.second,
                                      {"ref(samples)", "ref(sample_names)"});
          }
        }
      }
#if 0
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          ...
          // top level reference does not have dominator
          if (!QueryEngine->GetImmdiateLoopDominator(ref)) { continue; }
          CodeGen->EmitStmt("t_samplegen_"+node.second+".join()");
        }
      }
#endif
#if 0
      CodeGen->EmitComment("merge all samples for each reference to each array");
      for (auto node : SPSNodeNameTable) {
        if (RefTNode *ref = dynamic_cast<RefTNode *>(node.first)) {
          if (!QueryEngine->GetImmdiateLoopDominator(ref)) {
            continue;
          }
          CodeGen->EmitCode("for (auto sample : samples_"+node.second+") {");
          CodeGen->tab_count++;
#if 0
          CodeGen->EmitStmt("LAT_"+ref->getArrayNameString()+"[sample] = 0");
#endif
          CodeGen->EmitStmt("samples.insert(sample)");
          CodeGen->tab_count--;
          CodeGen->EmitCode("}");
        }
      }
...
#if 0
      for (auto array : ArrayNames) {
        params.push_back("samples_"+array);
      }
#endif
      params.push_back("samples");
      params.push_back("sample_names");
      params.push_back("start_iteration");
      CodeGen->EmitFunctionCall("sampler", params);
    }
  } else {
    CodeGen->EmitFunctionCall("pluss_timer_start", {});
...
void SamplerCodeGenProWrapperPass::SampleIterationCodeGen(RefTNode *TargetRef,
                                                          bool isFirstReference)
{

  if (isFirstReference) {
    CodeGen->EmitCode("void samplegen_" + SPSNodeNameTable[TargetRef] +
                        "(priority_queue<Iteration, vector<Iteration>, IterationComp> &samples,"
                      "unordered_set<string> &sample_names,"
                        "Iteration &start_iteration) {");
  } else {
    CodeGen->EmitCode("void samplegen_" + SPSNodeNameTable[TargetRef] +
                        "(priority_queue<Iteration, vector<Iteration>, IterationComp> &samples, unordered_set<string> &sample_names) {");
  }
  CodeGen->tab_count++;
  CodeGen->EmitStmt("unsigned s = 0");
//  CodeGen->EmitStmt("unordered_set<string> sample_names");
#if 0
  ...
  if (PerReferenceSampleCnt.find(TargetRef) != PerReferenceSampleCnt.end()) {
    ...
    CodeGen->EmitCode("while (s < NUM_SAMPLES_" + SPSNodeNameTable[TargetRef] + ") {");
  }
  CodeGen->tab_count++;
//  CodeGen->EmitLabel("BEGIN_SAMPLE_"+SPSNodeNameTable[TargetRef]);
  CodeGen->EmitStmt("string sample_label = \""+SPSNodeNameTable[TargetRef]+"\"");
  auto path_riter = path.rbegin();
  while (path_riter != path.rend()) {
...
//                                    induction_expr + ") + \"-\")");
  ...
  CodeGen->EmitCode("if (sample_names.find(sample_label) !=sample_names.end()) "
                      "{ continue; }");
  CodeGen->EmitStmt("sample_names.insert(sample_label)");
  path_riter = path.rbegin();
  ss << "Iteration sample(\"" << SPSNodeNameTable[TargetRef] + "\","
     << RefNodePriority[TargetRef] << ",";
  ss << "{";
...
#if 0
  ss << "uint64_t addr = GetAddress_" << SPSNodeNameTable[TargetRef] << "(";
  TranslateStatus status = SUCCESS;
  for (unsigned i = 0; i < TargetRef->getSubscripts().size(); i++) {
//          ss << "subscripts[" << i << "]";
    string tmp = ReplaceInductionVarExprByType(TargetRef->getSubscripts()[i], SAMPLE, status);
    if (status == SUCCESS)
      ss << tmp;
    if (i < TargetRef->getSubscripts().size()-1)
      ss << ",";
  }
  ss << ")";
  CodeGen->EmitStmt(ss.str());
  ss.str("");
#endif
//  CodeGen->EmitStmt("samples.insert(sample_label)");
  CodeGen->EmitStmt("samples.push(sample)");
  CodeGen->EmitStmt("s++");
  CodeGen->tab_count--;
  CodeGen->EmitCode("} // the end of while(s < NUM_SAMPLES)");
  CodeGen->tab_count--;
  CodeGen->EmitCode("} // end of sample_"+SPSNodeNameTable[TargetRef]+"()");
}

void SamplerCodeGenProWrapperPass::SamplerBodyGen(bool isPerReference)
{
    ...
    for (auto ref : SPSNodeNameTable) {
      ...
      auto path_riter = path.rbegin();
        ...
        if (EnableSampling && SamplingMethod == RANDOM_START)
          ...
          CodeGen->EmitStmt("int tid_to_run = 0");
        CodeGen->EmitComment("global counter");
        CodeGen->EmitStmt("uint64_t count = 0");
        CodeGen->EmitStmt("uint64_t addr = 0");
        CodeGen->EmitComment(
            "a hashmap stores all address of the sample.");
        CodeGen->EmitStmt("unordered_map<uint64_t, uint64_t> LAT");
        CodeGen->EmitComment(
            "stores all samples, the number of active sample is tracked using a "
            "separate count, active_samples");
        CodeGen->EmitStmt("unordered_set<string> samples");
        CodeGen->EmitDebugInfo(
            "unordered_map<uint64_t, Iteration *> LATSampleIterMap");
        //        LLVM_DEBUG(
        //            for (auto l: path) {
          ...
          if (LoopTNode *Loop = dynamic_cast<LoopTNode *>(*topiter)) {
            ...
            if (Loop != *(path.rbegin())) {
              ...
              if (Loop == *(path.rbegin())) {
                ...
                CodeGen->EmitStmt("Iteration start_iteration");
//                CodeGen->EmitStmt("string access = \"\"");
                CodeGen->EmitStmt("unordered_set<string> sample_names");
#if 0
              CodeGen->EmitStmt("mt19937 rng(dev())");
#endif
                ...
                CodeGen->EmitComment("generate all samples and sort "
                                     "them in order");
                if (PerReferenceSampleCnt.find(TargetRef) !=
                    PerReferenceSampleCnt.end()) {
                  CodeGen->EmitStmt(
                      "int num_samples = " +
                      to_string(PerReferenceSampleCnt[TargetRef]));

                } else {
                  CodeGen->EmitStmt("int num_samples = NUM_SAMPLES_" +
                                    SPSNodeNameTable[TargetRef]);
                }
#if 0
              // If a loop bound is constant, we can declare the random number generators
                ...
                CodeGen->EmitCode("while (s < num_samples) {");
                CodeGen->tab_count++;
                CodeGen->EmitStmt("string sample_label = \"\"");
                stringstream ss;
                auto path_riter = path.rbegin();
                while (path_riter != path.rend()) {
                  LoopTNode *LoopInPath =
...
#endif
                  ss.str(
                      ""); // clear the stream to generate other sampling stmts
                ...
                ss << "int sample_" << induction_expr << " = (dist_"
                  ...
                  ss << "int sample_" << induction_expr << " = ("
                     << sample_upper_bound_expr << " == 0)?(" << loop_init_expr
                     << "):"
                     << "(rand()%(" << sample_upper_bound_expr << "))*("
                     << loop_step << ")+(" << loop_init_expr << ")";
                  CodeGen->EmitStmt(ss.str());
                  ss.str(
                      ""); // clear the stream to generate other sampling stmts
                  CodeGen->EmitDebugInfo(
                      "cout << \"sample_" + induction_expr + ": [\" << (" +
                      loop_init_expr + ") << \",\" << (" +
                      sample_upper_bound_expr + "*(" + loop_step + ")+" +
                      loop_init_expr + ") << \"] samples \" << sample_" +
                      induction_expr + " << endl");
                  //                  CodeGen->EmitStmt("sample_label += (to_string(sample_" +
                  //                                    induction_expr + ") + \"-\")");
                  path_riter++;
                ...
                CodeGen->EmitComment("avoid duplicate sampling");
                CodeGen->EmitCode("if (sample_names.find(sample_label)"
                ...
                ss << "},";
                if (LoopTNode *ParallelLoop =
                        QueryEngine->GetParallelLoopDominator(TargetRef)) {
                  pair<LoopTNode *, LoopTNode *> IndvarDepResult =
                      CheckDependenciesInPath(ParallelLoop, path);
                  string induction_expr =
                      InductionVarNameTable[ParallelLoop->getInductionPhi()];
                  ...
                  AccPath pathTmp;
                  QueryEngine->GetPath(ParallelLoop, nullptr, pathTmp, false);
                  ss << loop_init_expr << "," << loop_step << ",true,"
                     << pathTmp.size();
                } else {
                  ss << "0,1,false,0";
                }
                ...
                CodeGen->EmitCode("if (s == 0 || "
                                  "sample.compare(start_iteration) < 0) "
                                  "{ start_iteration = sample; }");
                ss << "addr = GetAddress_" << SPSNodeNameTable[TargetRef]
                   << "(";
                TranslateStatus status = SUCCESS;
                for (unsigned i = 0; i < TargetRef->getSubscripts().size();
                     i++) {
                  //          ss << "subscripts[" << i << "]";
                  string tmp = ReplaceInductionVarExprByType(
                      TargetRef->getSubscripts()[i], SAMPLE, status);
                  if (status == SUCCESS)
                    ss << tmp;
                  if (i < TargetRef->getSubscripts().size() - 1)
                    ss << ",";
...
#endif
                // CodeGen->EmitStmt("unsigned active_samples = samples.size()");
                CodeGen->EmitDebugInfo("cout << \"Total tracking \" << LAT.size() << \" samples\" << endl");
        ...
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          ...
          CodeGen->EmitCode("}");
          CodeGen->tab_count--;
          CodeGen->EmitCode("} // end of clean to progress array");
          ...
          CodeGen->tab_count--;
          CodeGen->EmitCode("} // end of clean to LATSampleIterMap");
    ...
    AccPath path;
    QueryEngine->GetPath(FirstAccess, NULL, path);
    auto topiter = getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>()
        .TreeRoot->neighbors.begin();
    stringstream ss;
    if (EnableSampling && SamplingMethod == RANDOM_START) {
      ss << "void sampler(priority_queue<Iteration, vector<Iteration>, IterationComp> &samples,unordered_set<string> &sample_names,";
#if 0
      for (auto array : ArrayNames) {
        ss << "unordered_map<uint64_t,uint64_t> &LAT_" << array << ",";
      }
#endif
      ss << "Iteration &start_iteration) {";
    } else {
      ss << "void sampler() {";
    }
    CodeGen->EmitCode(ss.str());
    ...
    if (EnableSampling && SamplingMethod==RANDOM_START) {
      CodeGen->EmitStmt("bool start_reuse_search=false");
      CodeGen->EmitStmt("unsigned traversing_samples = 0");
      CodeGen->EmitStmt("unordered_set<string> samples_meet");
    }
    for (auto array : ArrayNames) {
      CodeGen->EmitStmt(
          "unordered_map<uint64_t, uint64_t> LAT_"+array);
    }
    if (EnableSampling && SamplingMethod == RANDOM_START) {
      for (auto array : ArrayNames) {
        CodeGen->EmitStmt("unordered_map<uint64_t, Iteration *> LAT_" + array +
                          "_SampleIterMap");
      }
    } else {
      CodeGen->EmitHashMacro("#if defined(DEBUG)");
...
//    CodeGen->EmitStmt("string access = \"\"");
    ...
    if (EnableSampling && SamplingMethod == RANDOM_START) {
      CodeGen->EmitDebugInfo(
          "cout << \"Total \" << samples.size() << \" samples\" << endl");
      CodeGen->EmitCode("while (!samples.empty()) {");
      CodeGen->tab_count++;
      CodeGen->EmitLabel("START_SAMPLE");
      ...
      CodeGen->EmitStmt(
          "if (!worker_threads.empty()) { worker_threads.clear(); }");
      for (auto array : ArrayNames) {
        CodeGen->EmitStmt("if (!LAT_" +array+".empty()) { LAT_" +array+".clear(); }");
        CodeGen->EmitCode("if (!LAT_" +array+"_SampleIterMap.empty()) {");
        CodeGen->tab_count++;
        CodeGen->EmitCode("for (auto entry : LAT_"+array+"_SampleIterMap) {");
        CodeGen->tab_count++;
        CodeGen->EmitStmt("delete entry.second");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->EmitStmt("LAT_"+array+"_SampleIterMap.clear()");
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
      }
      CodeGen->EmitDebugInfo("cout << \"Start tracking sample \" << start_iteration.toString() << endl");
      ...
      while (path_riter != path.rend()) {
        ...
        string induction_expr =
            InductionVarNameTable[LoopInPath->getInductionPhi()];
        CodeGen->EmitStmt("int sample_" + induction_expr +
                          "_start=start_iteration.ivs[" + to_string(i) + "]");
        CodeGen->EmitStmt("int " + induction_expr + "_start=0");
        path_riter++;
        i++;
      }
    ...
    if (EnableSampling && SamplingMethod == RANDOM_START) {
      ...
      for (auto array : ArrayNames) {
        ...
        CodeGen->tab_count--;
        CodeGen->EmitCode("}");
        CodeGen->EmitDebugInfo("cout << \"["+array+"]: The RI of \" << LAT_"+array+".size() << \" samples not found\" << endl");
      }
    }
...
void SamplerCodeGenProWrapperPass::SamplerBodyGenImpl(SPSTNode *Root)
{
  if (LoopTNode *LoopNode = dynamic_cast<LoopTNode *>(Root)) {
    if (LoopNode->isParallelLoop()) {
      ...
      AccPath path;
      QueryEngine->GetPath(FirstAccess, nullptr, path);
    ...
    ss.str("");
    if (EnableSampling && SamplingMethod == RANDOM_START) {
      if (RefNode == FirstAccess)
        CodeGen->EmitStmt("if (!start_reuse_search) { start_reuse_search = true; }");
...
//          ss << "Iteration *access = nullptr";
...
#if 0
        ss << "access = \"" << SPSNodeNameTable[RefNode] << "\"";
        ...
        while (pathIter != path.rend()) {
          ...
          string nest_loop_induction = ReplaceInductionVarExprByType(NestLoop->getInductionPhi(), DEFAULT, status);
          ss << "+(to_string(" << nest_loop_induction << ")+\"-\")";
          pathIter++;
        }
        CodeGen->EmitStmt(ss.str());
        ss.str("");
...
//        ss << "subscripts[" << i << "]";
      ...
      CodeGen->tab_count++;
      CodeGen->EmitCode("if (LAT_"+RefNode->getArrayNameString()+"[addr] != 0) {");
      CodeGen->tab_count++;
#endif
      if (EnableSampling && SamplingMethod == RANDOM_START) {
        if (!path.empty()) {
          ...
          CodeGen->EmitCode("if (access->compare(start_iteration) == 0) {");
          CodeGen->tab_count++;
      ...
      if (EnableSampling && SamplingMethod == RANDOM_START) {
        CodeGen->EmitStmt("Iteration *src = LAT_"+RefNode->getArrayNameString()+"_SampleIterMap[LAT_"+RefNode->getArrayNameString()+"[addr]]");
      }
      CodeGen->EmitHashMacro("#if defined(REUSE_DEBUG)");
      CodeGen->EmitStmt("cout << \"[\" << reuse << \"] " + SPSNodeNameTable[RefNode] +"\" << endl");
      CodeGen->EmitHashMacro("#endif");
      ...
      if (EnableSampling && SamplingMethod == RANDOM_START) {
        ...
        CodeGen->EmitStmt("LAT_"+RefNode->getArrayNameString()+"_SampleIterMap.erase(LAT_"+RefNode->getArrayNameString()+"[addr])");
        CodeGen->EmitStmt("LAT_"+RefNode->getArrayNameString()+".erase(addr)");
        ...
        CodeGen->EmitHashMacro("#endif");
        CodeGen->EmitStmt("goto START_SAMPLE");
        CodeGen->tab_count--;
...
#if 0
        ss << "bool isSamplerTerminate = ";
        for (auto array_iterator = ArrayNames.begin();
             array_iterator != ArrayNames.end(); array_iterator++) {
          if (array_iterator != ArrayNames.begin())
            ss << " &&";
          ss << "(LAT_" << *array_iterator << ".empty())";
        }
        CodeGen->EmitStmt(ss.str());
        ss.str("");
#endif
//        CodeGen->EmitStmt("active_samples--");
//        CodeGen->EmitDebugInfo("if (active_samples == 0) { cout << \"Find RI of all samples, goto END_SAMPLE\" << endl; }");
//        CodeGen->EmitCode("if (active_samples == 0) { goto END_SAMPLE; }");
#if 0
        CodeGen->EmitDebugInfo("cout << \"sample remains \" << LAT_" +
                               RefNode->getArrayNameString() +
                               ".size() << endl");
#endif
//        CodeGen->EmitDebugInfo("cout << \"sample remains \" << active_samples << endl");
//        CodeGen->EmitStmt("LAT_" + RefNode->getArrayNameString() + ".erase(addr)");
#if 0
...
//        CodeGen->EmitCode("if (samples.find(access) != samples.end()) {");
//        CodeGen->tab_count++;
//        CodeGen->EmitDebugInfo("cout << \"Touch sample \" << access << endl");
//        CodeGen->EmitStmt("LAT_" + RefNode->getArrayNameString() +
//                          "[addr] = count");
//        CodeGen->tab_count--;
#if 0
        CodeGen->EmitCode("}// end of if (LAT[addr] != 0)");
#endif
//        CodeGen->EmitCode(
//            "} // end of if (samples.find(access) != samples.end())");
//        CodeGen->tab_count--;
//        CodeGen->EmitCode(
//            "} // end of if (samples.find(access) != samples.end())");
#if 0
        CodeGen->EmitCode(
            "} // end of if (LAT[addr].find(addr) != LAT[addr].end())");
#endif
      } else {
        CodeGen->tab_count--;
        CodeGen->EmitCode(
...
void SamplerCodeGenProWrapperPass::ParallelSamplerBodyGenImpl(SPSTNode *Root,
                                                           AccGraph *G,
                                                           Interleaving type)
{
  ...
  while (true) {
    ...
    AccPath TargetToParallelRoot;
    G->QueryEngine->GetPath(FirstAccess, nullptr, TargetToParallelRoot);
    AccPath pathToTreeRoot;
    ...
    if (GraphNodeIter == NodesInGraph.begin()) {
      ...
      codess.str("");
      if (EnableSampling && SamplingMethod == RANDOM_START) {
        if (find(TargetToParallelRoot.begin(), TargetToParallelRoot.end(),
                 ParallelLoopNode) != TargetToParallelRoot.end()) {
        ...
        CodeGen->EmitStmt(
          ...
          ParallelSamplerSamplingHeaderGen(ParallelLoopNode, G, FirstAccess,
                                           type, scheduling);
#if 0
        CodeGen->EmitStmt("dispatcher.setStartPoint(start_iteration.ivs[" +
          ...
          CodeGen->tab_count++;
          CodeGen->EmitStmt("goto INTERLEAVING_LOOP_" +
...
//      errs() << "#endif\n";
    ...
    if (BranchTNode *Branch = dynamic_cast<BranchTNode *>(currentNode)) {
      ...
      CodeGen->EmitCode(codess.str());
      if (EnableSampling && SamplingMethod == RANDOM_START) {
        if (currentNode == FirstAccess) {
          CodeGen->EmitCode("if (!start_reuse_search) { start_reuse_search = (tid_to_run == start_tid); }");
        }
        CodeGen->EmitCode("if (start_reuse_search) {");
        CodeGen->tab_count++;
      }
...
#if 0
          codess << "access = \"" << SPSNodeNameTable[RefNode] << "\"";
          auto pathIter = path.rbegin();
          TranslateStatus status = SUCCESS;
          while (pathIter != path.rend()) {
            LoopTNode *NestLoop = dynamic_cast<LoopTNode *>(*pathIter);
            ...
            codess << "+(to_string(" << nest_loop_induction << ")+\"-\")";
            pathIter++;
          }
#endif
...
#endif
        CodeGen->EmitStmt("uint64_t reuse = count - LAT_"+RefNode->getArrayNameString()+"[addr]");
        CodeGen->EmitDebugInfo("cout << \"[\" << reuse << \"] " + SPSNodeNameTable[RefNode] +"\" << endl");
        CodeGen->EmitFunctionCall("pluss_histogram_update", {"reuse", "1."});
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          CodeGen->EmitStmt("Iteration *src = LAT_"+RefNode->getArrayNameString()+"_SampleIterMap[LAT_"+RefNode->getArrayNameString()+"[addr]]");
          CodeGen->EmitDebugInfo(
          ...
          CodeGen->EmitCode("} // end of if (LAT.find(addr) != LAT.end())");
#if 0
          codess << "bool isSamplerTerminate = ";
          for (auto array_iterator = ArrayNames.begin();
               array_iterator != ArrayNames.end(); array_iterator++) {
            if (array_iterator != ArrayNames.begin())
              codess << " &&";
            codess << "(LAT_" << *array_iterator << ".empty())";
          }
          CodeGen->EmitStmt(codess.str());
          codess.str("");
#endif
...
//                            "[addr] = count");
//          CodeGen->tab_count--;
//          CodeGen->EmitCode("} // end of if (samples.find(access) != samples.end()) ");
#if 0
          CodeGen->EmitCode("} // end of if (LAT[addr] != 0)");
          CodeGen->tab_count--;
          CodeGen->EmitCode("} // end of if (LAT.find(addr) != LAT.end() ");
...
void SamplerCodeGenProWrapperPass::PerRefSamplerBodyGenImpl(SPSTNode *Root,
                                                         RefTNode *TargetRef)
{
  if (LoopTNode *LoopNode = dynamic_cast<LoopTNode *>(Root)) {
...
//      SamplerBodyGenImpl(Branch->neighbors[0]);
    ...
    if (QueryEngine->areAccessToSameArray(RefNode, TargetRef)) {
      ...
      if (!params.empty()) {
        AccPath path;
        QueryEngine->GetPath(RefNode, nullptr, path);
        if (path.empty() || RefNode == TargetRef) {
          // special case, the RefNode is not in a loop
//          ss << "Iteration *access = nullptr";
        } else {
#if 0
...
#if 0
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          CodeGen->EmitCode("if (LAT[addr] != 0) {");
          CodeGen->tab_count++;
          CodeGen->EmitDebugInfo("Iteration *src = LATSampleIterMap[LAT[addr]]");
        }
#endif
        ...
        if (EnableSampling && SamplingMethod == RANDOM_START) {
          ...
          CodeGen->EmitHashMacro("#if defined(DEBUG)");
          CodeGen->EmitStmt("cout << \"delete sample \" << src->toString() << endl");
          CodeGen->EmitStmt("delete src");
          CodeGen->EmitStmt("LATSampleIterMap.erase(LAT[addr])");
          CodeGen->EmitHashMacro("#endif");
#endif
          CodeGen->EmitStmt("active_samples --");
          CodeGen->EmitDebugInfo("cout << \"pop LAT[\" << addr << \"] = \" << LAT[addr] << \", remain:\" << active_samples << endl ");
          CodeGen->EmitStmt("LAT.erase(addr)");
          CodeGen->EmitComment("stop traversing if reuse of all samples"
                               "are found");
          CodeGen->EmitDebugInfo("if (active_samples == 0) { cout << \"Find RI for all samples, goto END_SAMPLE \" << endl; }");
          CodeGen->EmitCode("if (active_samples == 0) { goto END_SAMPLE; }");
          if (RefNode == TargetRef) {
#if 0
            CodeGen->tab_count--;
            CodeGen->EmitCode("} else {");
            CodeGen->tab_count++;
            CodeGen->EmitDebugInfo("cout << \"push LAT[\" << addr << \"] = \" << count << endl ");
            CodeGen->EmitStmt("LAT[addr] = count");
            CodeGen->EmitDebugInfo("LATSampleIterMap[count] = access");
            CodeGen->tab_count--;
            CodeGen->EmitCode("} // end of if (LAT[addr] != 0)");
            CodeGen->tab_count--;
            CodeGen->EmitCode("} else {");
            CodeGen->tab_count++;
            CodeGen->EmitDebugInfo("delete access");
#endif
            CodeGen->EmitCode("}");
            CodeGen->EmitCode("if (samples.find(access) != samples.end()) {");
            CodeGen->tab_count++;
            CodeGen->EmitDebugInfo(
                "cout << \"push LAT[\" << addr << \"] = \" << count << endl ");
            CodeGen->EmitStmt("LAT[addr] = count");
            CodeGen->tab_count--;
            CodeGen->EmitCode(
                "} // end of if (samples.find(access) != samples.end())");
...
void SamplerCodeGenProWrapperPass::PerRefParallelSamplerBodyGenImpl(SPSTNode *Root,
...
//            codess << "subscripts[" << i << "]";
          ...
          if (EnableSampling && SamplingMethod == RANDOM_START) {
            ...
            CodeGen->EmitDebugInfo("cout << \"pop LAT[\" << addr << \"] = \" << LAT[addr] << \", remain:\" << LAT.size()-1 << endl ");

            CodeGen->EmitStmt("LAT.erase(addr)");
            CodeGen->EmitComment("stop traversing if reuse of all samples"
                                 "are found");
            CodeGen->EmitDebugInfo("if (LAT.empty()) { cout << \"[\" << reuse << \"] for last sample \" << endl; }");
            CodeGen->EmitCode("if (LAT.empty()) { goto END_SAMPLE; }");
#endif
            CodeGen->EmitStmt("active_samples--");
            CodeGen->EmitDebugInfo("cout << \"pop LAT[\" << addr << \"] = \" << LAT[addr] << \", remain:\" << active_samples << endl ");
...
void SamplerCodeGenProWrapperPass::LoopIterUpdateGen(AccGraph *G,
                                                  AccGraphEdge *E,
                                                  LoopTNode* IDomLoop,
...
void SamplerCodeGenProWrapperPass::ParallelSamplerSamplingHeaderGen(LoopTNode *ParallelLoop,
                                                                 AccGraph *G,
                                                                 RefTNode *TargetRef,
                                                                 Interleaving type,
...
SamplerCodeGenProWrapperPass::CheckDependenciesInPath(LoopTNode *Child, AccPath &Path)
{
...
bool SamplerCodeGenProWrapperPass::hasInductionVarDependenceChildren(LoopTNode *LoopNode)
{
  IVDepNode *ParentNode = nullptr;
  for (auto ivdep : getAnalysis<PlussLoopAnalysis::LoopAnalysisWrapperPass>()
      .InductionVarDepForest) {
...
bool SamplerCodeGenProWrapperPass::hasParallelLoopInductionVar(Value *V)
{
  bool retVal = false;
  if (!V) {
    return false;
...
bool SamplerCodeGenProWrapperPass::isInParallelLoop(RefTNode *RefNode)
{
  AccPath path;
  QueryEngine->GetPath(RefNode, nullptr, path);
...
bool SamplerCodeGenProWrapperPass::runOnFunction(Function &F) {
  if (!FunctionName.empty() &&
      F.getName().str().find(FunctionName) == string::npos) {
  ...
  PointerAliasPairs = getAnalysis<TreeAnalysis::PlussAbstractionTreeAnalysis>()
  ...
  ModelValidateLoops = getAnalysis<ModelAnalysis::ModelValidationAnalysis>()
                           .ModelApplicableLoops;
  int priority = getAnalysis<TreeAnalysis::PlussAbstractionTreeAnalysis>().RefNodeOrderedList.size();
  for (auto refnode :
       getAnalysis<TreeAnalysis::PlussAbstractionTreeAnalysis>().RefNodeOrderedList) {
    RefNodePriority[refnode] = priority;
    priority--;
  }
  ...
  for (auto entry :SPSNodeNameTable) {
    if (RefTNode *RefNode = dynamic_cast<RefTNode *>(entry.first)) {
      if (ArrayNames.find(RefNode->getArrayNameString()) == ArrayNames.end()) {
        ArrayNames.insert(RefNode->getArrayNameString());
      }
    }
  }
  ...
  // If EnableModelOpt option is given, the model will be applied if the parallel loop 1) can be parallelized by the OpenMP static scheduling and 2) threads are interleaved uniformly Here we check the condition 2)
  if (EnableSampling) {
  ...
  if (EnableSampling && SamplingMethod == RANDOM_START) {
    ...
    if (FirstLoop) {
      SmallVector<SPSTNode *, 8> FirstAccesses = QueryEngine->GetFirstAccessInLoop(FirstLoop);
      for (auto entry : SPSNodeNameTable) {
        if (RefTNode *RefNode = dynamic_cast<RefTNode *>(entry.first)) {
          if (!QueryEngine->GetImmdiateLoopDominator(RefNode))
            continue;
          bool isFirstReference = false;
          if (find(FirstAccesses.begin(), FirstAccesses.end(), RefNode) != FirstAccesses.end()) {
            isFirstReference = true;
            FirstAccess = RefNode;
          }
          SampleIterationCodeGen(RefNode, isFirstReference);
        }
      }
    }
  }
  SamplerBodyGen(EnableParallelOpt);
  MainFuncGen();
  return false;
}

void SamplerCodeGenProWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
