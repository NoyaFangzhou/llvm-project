...
//                                    induction_expr + ") + \"-\")");
              ...
              CodeGen->EmitCode("if (!LAT.empty()) {");
              CodeGen->tab_count++;
              CodeGen->EmitCode("for (unsigned i = 0; i < LAT.size(); i++) {");
              CodeGen->tab_count++;
              CodeGen->EmitComment("Samples with no reuse are logged as RI = -1");
              CodeGen->EmitFunctionCall("pluss_histogram_update", {"-1", "(double)LAT[i].size()"} );
              CodeGen->tab_count--;
              CodeGen->EmitCode("}");
